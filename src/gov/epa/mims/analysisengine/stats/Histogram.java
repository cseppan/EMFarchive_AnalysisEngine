...
/**
 * <p>Description: This class accepts a DataSetIfc and a set of bins and
 * produces a LabeledDataSetIfc that is histogram of the provided data broken
 * up by the provided bins. </p>
 * <p>Copyright: Copyright (c) 2003</p>
 * <p>Company: UNC-CH, Carolina Environmental Program</p>
 * @author Daniel Gatti
 * @version $Id: Histogram.java,v 1.1 2005/09/19 14:16:05 rhavaldar Exp $
 */
public class Histogram
{
   /** The constant to be used for histograms that return the number of
    * items in each bin.*/
   public static final byte FREQUENCY = -55;

   /** The constant to be used for histograms that return the percentage of
    * items in each bin. */
   public static final byte PERCENTAGE = -56;

   /** The constant to be used for histograms that return the probability of
    * items in each bin.*/
   public static final byte PROBABILITY = -57;

   /** A pass through constant for the overflow bin in the colt Histogram
    * implementation. */
   public static final int OVERFLOW_BIN = Histogram1D.OVERFLOW;

   /** A pass through constant for the underflow bin in the colt Histogram
    * implementation. */
   ...
   /**
    * The main Histogram method. Call this method with a DataSetIfc, a set of
    * bins and an option that requests frequency or percentage. It will return
    * a DataSetIfc with the resulting Histogram.
    * Histogram1D returns a two additional bins '-Inf to bins[0]' and 'bin[last] to Inf'
    * return label data set will only have these two additional bins if and only these
    * bins have values other than 0
    * @param dataSet DataSetIfc to use to create the histrogram.
    * @param bins double[] with the breakpoints between bins.
    * @param format DecimalFormat which will be applied to the format the labels
    * can be null
    * @param type byte that is either FREQUENCY, PROBABLILITY or PERCENTAGE.
    * @see Histogram1D
    */
...
//System.out.println("hist.binEntries("+i+")="+hist.binEntries(i) + " binLabels["+(i+1)+"]= " + binLabels[i+1]);                        
         }
         int lastBinEntry = hist.binEntries(OVERFLOW_BIN);
//System.out.println("lastBinEntry="+lastBinEntry + " binLabels[last]= " + binLabels[binLabels.length - 1]);         
         if(lastBinEntry >0)
         {
      ...
      {
         double total = (double)hist.allEntries();
         // Extract the bin values and return them in DataSetIfc.
         int firstBinEntry = hist.binEntries(UNDERFLOW_BIN);
         if(firstBinEntry >0)
         {
         ...
         for(int i = 0; i < bins.length - 1; i++)
         {
            result.addData((double)hist.binEntries(i) / total * 100.0,
               binLabels[i+1]);
         }
         int lastBinEntry = hist.binEntries(OVERFLOW_BIN);
         if(lastBinEntry >0)
      ...
      else if (type == PROBABILITY)
      {
         ...
         for(int i = 0; i < bins.length - 1; i++)
         {
            result.addData((double)hist.binEntries(i) / total,
               binLabels[i+1]);
         }
         int lastBinEntry = hist.binEntries(OVERFLOW_BIN);
         if(lastBinEntry >0)
