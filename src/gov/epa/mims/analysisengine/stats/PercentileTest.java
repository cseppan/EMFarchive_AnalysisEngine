...
/**
 * <p>Description: Tests for the Percentile class. </p>
...
public class PercentileTest extends TestCase
{
   ...
   public void testUniformDisribution()
   {
      ...
      try
      {
         LabeledDoubleSeries result =
            (LabeledDoubleSeries)Percentile.generate(ds2, percentiles);
         result.open();
         for (int i = 0; i < result.getNumElements(); i++)
         {
...
//System.out.println(result.getLabel(i) + " : " + result.getElement(i));
   ...
   public void testLargeNumbers()
   {
      ...
      {
         LabeledDoubleSeries result =
            (LabeledDoubleSeries)Percentile.generate(ds3, percentiles);
         result.open();
         for (int i = 0; i < result.getNumElements(); i++)
         {
//System.out.println(result.getLabel(i) + " : " + result.getElement(i));
   ...
   public void testLargeDataSet()
   {
      ...
      double[] percentiles = {0.000001, 0.00001, 0.0001, 0.001, 0.01, 0.1, 0.25,
         0.5, 0.75, 0.90, 0.99, 0.999, 0.9999, 0.99999, 0.999999};
      double[] answers = {1.0, 10.0, 100.0, 1000.0, 10000.0, 100000.0, 250000.0,
         500000.0, 750000.0, 900000.0, 990000.0, 999000.0, 999900.0, 999990.0,
         999999.0};

      try
      {
         LabeledDoubleSeries result =
            (LabeledDoubleSeries)Percentile.generate(ds4, percentiles);
         result.open();
         for (int i = 0; i < result.getNumElements(); i++)
         {
//System.out.println(result.getLabel(i) + " : " + result.getElement(i));
   ...
   public void testLabels()
   {
      ...
      {
         LabeledDoubleSeries result =
            (LabeledDoubleSeries)Percentile.generate(ds5, percentiles);
         result.open();
         for (int i = 0; i < result.getNumElements(); i++)
         {
//System.out.println(result.getLabel(i));
   ...
   public void testNormalDistribution()
   {
      ...
      {
         LabeledDoubleSeries result =
               (LabeledDoubleSeries)Percentile.generate(ds6, percentiles);
         result.open();
         for (int i = 0; i < result.getNumElements(); i++)
         {
//System.out.println(result.getLabel(i) + " : " + result.getElement(i) + " " + normal.cdf(result.getElement(i))+" "+percentiles[i]);
            ...
            // same algorithm.
            assertEquals("percentile " + i, normal.cdf(result.getElement(i)),
               percentiles[i], 0.01);
         }
         result.close();
      }
   ...
   public void testNegativeNumbers()
   {
      ...
      {
         LabeledDoubleSeries result =
               (LabeledDoubleSeries)Percentile.generate(ds7, percentiles);
         result.open();
         for (int i = 0; i < result.getNumElements(); i++)
         {
