 ...
 * produces a LabeledDataSetIfc that lists the percentiles in the provided bins. </p>
...
public class Percentile
{
   ...
   /**
     * Given a set of data and a list of quantiles, return the value of each
     * percentile.
     * Colt requires that the data be sorted first, so we have to copy the
     * values from the DataSet and sort them.
     *
     * @param dataSet DataSetIfc that is the data to partition.
     * @param percentiles double[] with values between 0.0 and 1.0 that indicate
     *    the percentiles to use.
     */
   public static LabeledDataSetIfc generate(DataSetIfc dataSet,
          double[] percentiles)
          throws Exception
   {
     ...
     /**
      * RP: A Temporary method: For the TABLE readers to read the percentile labels as
      * doubles
      */
   public static LabeledDataSetIfc generate(DataSetIfc dataSet,
          double[] percentiles, boolean table)
          throws Exception
   {
      // First verify that we have percentiles between 0.0 and 1.0.
      for (int i = 0; i < percentiles.length; i++)
