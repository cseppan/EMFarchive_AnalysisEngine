   ...
   ChildHasChangedListener
{
   ...
   /** The status bar at the bottom of the table that contains the number of
    * rows and columns. */
   ...
   /** A list of all of the Formatters for the columns. This is used to pass to
    * the filter mechanism. */
   //   protected Format[] allColumnFormats = null;
   
   /** The current X value for the last mouse click. Used to bring up the popup
    * menu. */
   ...
   /**
    * Constructor.
    * @param model MultiRowHeaderTableModel that is the base model for this table.
    */
   public SortFilterTablePanel(Component parent, MultiRowHeaderTableModel model)
   {
      ...
      {
         ColumnFormatInfo info = new
         ColumnFormatInfo(table.getColumnModel().getColumn(i));
//System.out.println("table.getColumnName(i)="+table.getColumnName(i));         
   ...
   /**
    * Adding/Removing different actions are implemented in createPopupMenuAndToolBar
    * This is only implemented in the gov.epa.emissions.emisview.gui.SortFilterSelectionPanel
    * So if you want to use this constructor from some other class please refactor
    *the createPopupMenuAndToolBar()
    *
    */
   public SortFilterTablePanel(Component parent, MultiRowHeaderTableModel model,
   boolean popupMenu,boolean sort, boolean filter, boolean format, boolean showHide, boolean reset)
   {
      this.sort = sort;
      this.filter = filter;
      this.format= format;
   ...
   /** A helper method to create icons
    * subclasses will override to add additional icons
    */
   protected void createIcons()
   {
      // //Icons for the tool bar ////////
      sortIcon = createImageIcon(
      "/gov/epa/mims/analysisengine/table/icons/sort.jpeg");
      filterIcon = createImageIcon(
      "/gov/epa/mims/analysisengine/table/icons/filter.jpeg");
      showHideIcon = createImageIcon(
      "/gov/epa/mims/analysisengine/table/icons/show.jpeg");
      resetIcon = createImageIcon(
      "/gov/epa/mims/analysisengine/table/icons/reset.jpeg");
      formatIcon = createImageIcon(
      "/gov/epa/mims/analysisengine/table/icons/format.jpeg");
      plotIcon = createImageIcon(
      "/gov/epa/mims/analysisengine/table/icons/plot.jpeg");
      statsIcon = createImageIcon(
      "/gov/epa/mims/analysisengine/table/icons/histogram.jpeg");
      topNRowsIcon = createImageIcon(
      "/gov/epa/mims/analysisengine/table/icons/topNRows.jpeg");
      bottomNRowsIcon = createImageIcon(
      "/gov/epa/mims/analysisengine/table/icons/bottomNRows.jpeg");
      configIcon = createImageIcon(
      "/gov/epa/mims/analysisengine/table/icons/config1.jpeg");
      clearAllIcon = createImageIcon(
      "/gov/epa/mims/analysisengine/table/icons/clearAll.jpeg");
      selAllIcon = createImageIcon(
      "/gov/epa/mims/analysisengine/table/icons/selectAll.jpeg");
   }
    ...
    * Also add any items that are not column specific to the toolbar.
    */
    ...
    * @param model AbstractTableModel that is the base model for this table.
    */
...
//      });
      ...
      /*allColumnFormats = new Format[overallModel.getColumnCount()];
      for (int i = 0; i < allColumnFormats.length; i++) {
         overallModel.setColumnFormatInfo(table.getColumnName(i), new
ColumnFormatInfo(table.getColumnModel().getColumn(i)));
         allColumnFormats[i] = table.formats[i];
      }*/
   } // createTable()
   ...
   /**
    * WARNING:When you try to set the data values it should be done through the overallTable mode
   ...
   public void reset()
   {
      ...
      for(int i=1; i< model.getColumnCount(); i++)
      {
         ...
         if(!colClass.equals(Boolean.class))
         {
            ...
            getDefaultFormattedCellRenderer(colClass));
         }
         overallModel.setColumnFormatInfo(table.getColumnName(i), new
         ColumnFormatInfo(column));
      }
      table.setRowHeight(defaultRowHeight);
   } // reset()
   
   /**
    * Show the GUI for formatting the column text and update the selected
    * column. Also reset the row hieghts in the table to match any new
    * font heights.
    */
   public void showColumnFormatGUI()
   {
      ...
      if (selectedColumn > 0)
      {
         ...
         if(colClass.equals(Boolean.class))
         {
            ...
            "You cannot format this column",UserInteractor.ERROR);
            return;
         }
         ...
         ColumnFormatGUI formatGUI = new ColumnFormatGUI((JFrame)parent,
         ...
         try
         {
            ...
            FormattedCellRenderer formattedRenderer = new FormattedCellRenderer(
            singleFormatInfo.getFormat(),singleFormatInfo.alignment);
            ...
            overallModel.setColumnFormatInfo(table.getColumnName(selectedColumn),
            singleFormatInfo);
         ...
         catch (Exception e)
         {
            ...
            "Unable to get formatting information", e,
   ...
   public void showFilterRowGUI()
   {
      ...
      FilterRowGUI filterGUI = new FilterRowGUI((JFrame)parent, cols, classes,
      overallModel.getFilterCriteria());
   ...
   /**
    * Present a GUI that allows the user to select a subset of columns
    * and allows them to format the selected columns.
    *
    */
   ...
   public void showMultipleColumnFormatGUI()
   {
      ...
      else
      {
         ...
         formatGUI = new ColumnFormatGUI((JFrame)parent, columnHeaders, selected, formats,
         formatInfo);
      }// else
      ...
      try
      {
         ...
         for (int c = 0; c < selectedColumns.length; c++)
         {
            if (selectedColumns[c])
            {
               ...
               // Now set the new renderer to be used in the column.
               column.setCellRenderer(formattedRenderer);
               overallModel.setColumnFormatInfo(
               table.getColumnName(c+1), new ColumnFormatInfo(column));
            } // if (selectedColumns[c])
         } // for(c)
         // Check the size of all of the fonts. Set the row height to be the
         // height of the largest font in the table.
      ...
      {
         DefaultUserInteractor.get().notifyOfException(this,
         "Unable to get formatting information", e, UserInteractor.ERROR);
      } // catch
   } // showMultipleColumnFormatGUI()
   
   /**
    * Show the show/hide columns GUI. Filter the table based on the user's
   ...
   public void showFilterColumnsGUI()
   {
      ...
      {
         filterGUI = new SelectColumnsGUI((JFrame)parent, filterCriteria, "Show/Hide Columns",
         "Show", "Hide");
     ...
     /*    Format[] allColumnFormats = new Format[numcols];
         TableColumnModel model = table.getColumnModel();
         for(int i=1; i < numcols ; i++)
         {
               allColumnFormats[i-1]=((FormattedCellRenderer)model.getColumn(i
   ...
   public void showSortGUI()
   {
      ...
      if (sortGUI.getResult() == OptionDialog.OK_RESULT)
      {
         ...
         /*
          // First we have to make sure that the columns we are sorting by are
          // visible.
          ...
          if (filterCriteria != null)
          {
          boolean[] colsToShow = filterCriteria.getColumntoShow();
          for (int i = 0; i < columnIndices.length; i++)
          {
          if(colsToShow[columnIndices[i]] == false)
          {
          DefaultUserInteractor.get().notify("Invalid Column for Sorting",
          "You cannot apply a sort to a column that is not being displayed.\n" +
          "Please show the column and then sort on it.", UserInteractor.ERROR);
          return;
          }
          } // for(i)
   ...
   /**
    * Find the currently selected column and sort it.
    * This is what is called when the user clicks on the header of the table.
    * @param ascending boolean that is true if we should sort in ascending order.
    */
   private void sortAtCurrentX(boolean ascending)
   {
      ...
      if (selectedColumn > 0)
      {
         ...
         SortCriteria sortCriteria = new SortCriteria(columnNames,
         new boolean[]
    ...
    * @param sortCriteria SortCriteria to use to sort the data.
    */
   protected void sort(SortCriteria sortCriteria)
   {
      overallModel.sort(sortCriteria);
   } // sort()
   ...
   protected void updateFormat()
   {
      ...
      for(int i=1; i< count; i++)
      {
         ...
         {
            FormattedCellRenderer renderer = new
            FormattedCellRenderer(info.getFormat(),info.alignment);
   ...
   /**
    * After an operation that changes the number of rows or columns, update
    * the number of rows and columns in the status label.
    */
   public void updateStatusLabel()
   {
      statusLabel.setText(
      overallModel.getRowCount() + ROWS_STR
      + overallModel.getDataColumnCount() + COLUMNS_STR);
   } // updateStatusLabel()
   
   /** This will apply the filter criteria to the TableModel
    *WARNING: Please note that we are not checking whether filterCriter has the
    *same column names as overall table model
   ...
   public void showSaveConfigGUI()
   {
      ...
      {
         new GUIUserInteractor().notify(this,"Saving Configuration?", e.getMessage(),
               UserInteractor.NOTE);
         e.printStackTrace();
      }
   }//showSaveConfigGUI
...
//            aconfig.getModel()); // updated model ???? RP
         ...
         LoadConfigurationGUI gui = new LoadConfigurationGUI(config, aconfig,
            table, (JFrame)this.parent);
         gui.show();
      }
      catch (Exception e)
      {
         new GUIUserInteractor().notify(this,"Load Configuration", e.getMessage(),
               UserInteractor.ERROR);
         e.printStackTrace();
      }
   }//showLoadConfigGUI
   ...
   public static void main(String[] args)
   {
      ...
      outer:
         ...
         {
            { "String 1", "String 2", "Integer", "Double", "Date"},
            { "g/cm3", "lb/in^2", "m/s", "items/hectare", "years"}
         };
         final SimpleTestModel tableModel = new SimpleTestModel(data, null,
         columnHeaders);
 ...
 // main()
   ...
   class LoadConfigAction extends AbstractAction
   {
      ...
      public void actionPerformed(ActionEvent e)
      {
         ...
         do
         {
            ...
            {
               new GUIUserInteractor().notify(SortFilterTablePanel.this,"Error loading file",
   ...
   protected class PopupMouseAdapter extends MouseAdapter
   {
      ...
      public void mousePressed(MouseEvent e)
      {
         ...
         {
            popupMenu.show(SortFilterTablePanel.this,
            point.x - scrollPane.getViewport().getViewPosition().x,
