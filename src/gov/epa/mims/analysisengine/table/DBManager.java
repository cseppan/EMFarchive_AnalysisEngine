...
public class DBManager
{
   ...
   /** a hash map of db names (String) to Connections */
   private static HashMap connections = new HashMap(10);

   /**
    * opens a connection to a particular database
   ...
   /**
    * utility method
    * given a resultset print it out
    */
   ...
   /**
    * gets a list of database names using the <query>SHOW DATABASES</query> command
    * @return String[] containing the database names
    * @throws Exception if error on executing query
    */
    ...
    * @param tableColumn the combination of table and column where to get the
    * results from
    * @return a list of distinct values
    * @throws Exception
    */
   public static String[] getDistinctValues(String dbName, String tableColumn)
      throws Exception
   {
     Connection connection = openConnection(dbName);
     try
     {
       ...
       ResultSet rs = connection.createStatement().executeQuery("SELECT DISTINCT "
           +tableColumn+" FROM "+tableColumn.substring(0, tableColumn.indexOf('.')));
       rs.last();
     ...
     catch(SQLException sqle)
     {
       ...
       // connection
       throw new Exception("Error occurred while getting distinct values from "
                           +tableColumn+"\n"+sqle.getMessage());
     }
   }

    /** check whether tables of the dbName data base contains data
     * @param dbTables String [] array of required table not all the table in the database @see DaveContants DB_TABLE...
     ...
     * @param tableName
     */
    private static void doesTableContainData(String dbName, String tableName)
       throws Exception
    {
       Connection connection = (Connection)connections.get(dbName);
...
//    }
  ...
  **/
  public static void createTable(String dbName,String table, String[] colNames,
   String[] colTypes, String primaryCol, boolean auto) throws Exception
  {
...
System.out.println("create table : " + queryString);
  ...
  /** check whether a table exist with in a database
   * ASSUMPTION tableName string should be exact name of the file
...
System.out.println("tableName@isTableExist="+tableName);
   ...
   * @param dbName where table will be created
   * @param origTableName where data will be copied from
   * @param newTableName  table will be created data will be copied to
   */
  public static void copyTable(String dbName,String origTableName,
      String newTableName ) throws Exception
  {
    Connection conection = openConnection(dbName);
    String query = "CREATE TABLE " + newTableName + " SELECT * FROM "
      + origTableName;
System.out.println("query=" + query);
    conection.createStatement().execute(query);
  }//copyTable
  
   /** A helper method to retreive the info from metrics table of each type
    * WARNING: This method is written with the knowlege of the metrics table schema
    * desCol=description, metricsDescCol='metric_desc' metricsName='acr_risk' OR 'sci'....
    * WARNING:If used for other table, won't give any useful results.
    * @param desTable a table from description will be fetched
    * @param desCol a column in which description is exist
    * @param optionCol to narrow down resultset or a column is used to specify a
    * condition
    * @param optionString a String to match a string in optionCol
    * @return String db info string
    */
   public static  String getDbInfo(String dbName, String desTable, String desCol,
      String metricsDescCol, String metricsName) throws Exception
   {
      ResultSet rs = null;
      String info = null;
      try
      {
         ...
         rs = statement.executeQuery("SELECT " + desCol + " FROM " + desTable +
         " WHERE " +  metricsDescCol + " = '"+metricsName+"'");
         if(rs.next())
         {
      ...
      {
         //e.printStackTrace();
         throw new Exception("Error in getting DB info for db="+dbName+
            " and table=" +desTable);
      }
      return info;
