 ...
 * @version $Id: HistogramGUI.java,v 1.1 2005/09/19 14:14:04 rhavaldar Exp $
 */
...
public class HistogramGUI extends JPanel
{
   ...
   private void initialize()
   {
      ...
      JLabel minDataValueLabel = new JLabel(format.format(
      histogramModel.getMinValue()));
      ...
      dataRangePanel.setBorder(BorderFactory.createTitledBorder(
      BorderFactory.createLineBorder(Color.black),"Data Range"));
      ...
      hitogramTypePanel.setBorder(BorderFactory.createTitledBorder(
      ...
      {HistogramModel.DEFAULT_EFORMAT};
   ...
   /** Recompute the bin size base on the lower bound, upper bound and the no
    * of bins
    */
   private void recompute()
   {
      ...
      if(equalBinRB.isSelected())
      {
         ...
         {
            binBreakPoints = histogramModel.createEqualSizeBins(noOfBinsOrSteps,
            minValue, maxValue);
         }//else
      }//if(equalBinRB.isSelected())
      else if(powerTenRB.isSelected())
      {
         binBreakPoints = histogramModel.createFactorTenBins(
         noOfBinsOrSteps, minValue);
      }//else if(powerTenRB.isSelected())
      else
      {
         DefaultUserInteractor.get().notify(this,"Error",
         "This is not a valid option", UserInteractor.ERROR);
      }//else
   ...
   public void initGUIModel()
   {
      ...
      {
         DefaultUserInteractor.get().notify(this,"Error","The histogram type "
         +histogramType + " is not in the gui yet! ",
         UserInteractor.ERROR);
      ...
      double [] breakPoints =
      histogramModel.createEqualSizeBins(noOfBins,minValue,maxValue);
      if(equalBinRB.isSelected())
      {
   ...
   public HistogramModel saveToModel()
   {
      ...
      {
         DefaultUserInteractor.get().notify(this,"No Bins",
         "Spefify bin break points", UserInteractor.ERROR);
         //don't close
         return null;
      }//if(bins == null || bins.length == 0)
   ...
   public static void main(String[] args)
   {
      ...
      gov.epa.mims.analysisengine.gui.LabeledDoubleSeries lds =
      new gov.epa.mims.analysisengine.gui.LabeledDoubleSeries();
