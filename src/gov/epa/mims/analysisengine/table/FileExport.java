...
/**
 * FileExport.java
 * Use to write a file , given a file name and delimiter and the data for the file 
 *
...
//   private String fileFooter;
   ...
   public void writeFile(String fileHeader,String [] columnRowHeaders,
      String[][] columnHeaders, ArrayList fileData,String fileFooter) throws Exception
   {
      try
      {
      ...
      {
         DefaultUserInteractor.get().notify(null,"Error opening output file",
         "Could not open file for export: " + fileName, UserInteractor.ERROR);
      }
    ...
    * helper method to print the data column header data 
    * WARNING: 
    * Assumed columnHeader!= null and elements of this 2D array also not null
    * and columnHeaders.length == fileData.size() ie column sizes are equal
    * 
    */
   ...
   private void printData(TableModel tableModel)
   {
      ...
      for(int i=0; i< rowCount; i++ )
      {
         for(int j=0; j<colCount-1; j++)
         {
            ...
            boolean flag = tableModel.getColumnClass(j).equals(Double.class)
               && ((Double)obj).isNaN();
            String objString = (flag) ? SignificantDigitsFormat.NaN_FORMAT : obj.toString();
            printAWordPlusDelimiter(objString);
         }//for(j)
         Object obj = tableModel.getValueAt(i,colCount-1);
         boolean flag = tableModel.getColumnClass(colCount-1).equals(Double.class)
               && ((Double)obj).isNaN();
         String objString = (flag) ? SignificantDigitsFormat.NaN_FORMAT : obj.toString();
   ...
   /** helper method to print the data without quotes and followed by End of Line
    */
