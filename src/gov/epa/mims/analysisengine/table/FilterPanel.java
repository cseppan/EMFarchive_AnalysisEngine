...
/**
 * <p>Description: A generic panel that displays and edits filtering information.
 *   There is a table with 3 columns. The first column is the "what to sort on"
 *   column.  This might be a column name for row filtering. The second column
 *   is the "what operation to use when filtering" column. This might contain
 *   "greater-than" or "contains".  The last column is the "what to look for"
 *   column. So, to filter rows with emissions greater than 1000, the user
 *   enters "Emission", ">", "1000".
 *   At the top of the panel are two radio buttons for "AND" and "OR" comparison
 *   of all of the filtering criteria entered. </p>
...
public class FilterPanel extends JPanel implements ChildHasChangedListener, ActionListener
{
   ...
   /** The index of the default filtering choice to show when a new row is
    * added to the table. */
   protected int defaultFilteringChoice = 0;

   /** The index of the default operation choice to show when a new row is
    * added to the table. */
   ...
   /**
    * Constructor.
    * @param useAnd boolean that is true if the "AND" button should be checked.
    * @param newData String[][] that is all of the data to place in the table.
    * @param filteringChoices String[] with all of the choices for the first
    *    column.
    * @param operationChoices String[] with all of the choices for the operation
    *    column.
    * @param defaultFilteingChoice int that is the index of the default
    *    filtering choice to display when a new row is added.
    * @param defaultOperationChoice int that is the index of the default
    *    operation choice to display when a new row is added.
    */
   public FilterPanel(boolean useAnd, String[][] newData,
                      String[] filteringChoices, String[] operationChoices,
                      int defaultFilteingChoice, int defaultOperationChoice)
   {
      initialize();
      setFilteringChoices(filteringChoices, operationChoices,
                          defaultFilteingChoice, defaultOperationChoice);
      setUseAnd(useAnd);
...
//   }
    ...
    * @return boolean that is true if filtering by "AND" and false if filtering
    *    with "OR".
    */
   ...
   protected void initialize()
   {
      ...
      localModel = new DefaultTableModel(new Object[0][0], new String[] {"Column Name",
         "Operation", "Value"});
  ...
  public void actionPerformed(ActionEvent e)
  {
   ...
   /**
    * Set the choices that the user is offered in the first and operation
    * columns.
    * @param filteringChoices String[] that are the choices to place in the first
    *    column.
    * @param operationChoices String[] that are the choices to place in the
    *    operation column.
    * @param int defaultFilteringChoice that is the index of the default
    *    filtering choice to display when a new row is added.
    * @param int defaultOperationChoice that is the index of the default
    *    operation choice to display when a new row is added.
    */
   public void setFilteringChoices(String[] filteringChoices,
      String[] operationChoices, int defaultFilteringChoice,
      ...
      if (table != null)
      {
         ...
         if (filteringChoices != null)
         {
            ...
            else
            {
               ...
               columnModel.getColumn(NAME_COLUMN).setCellEditor(
                     new DefaultCellEditor(columnBox));
            }
         } // if (filteringChoices != null)
         ...
         if (operationChoices != null)
         {
            ...
            columnModel.getColumn(OPERATION_COLUMN).setCellEditor(
                  new DefaultCellEditor(operationBox));
         } //if (operationChoices != null)
      } // if (table != null)
   } // setFilteringChoices()
   ...
   /**
    * Return true if the "AND" button is checked.
    * @param boolean that is true if filtering by "AND" and false if filtering
    *    with "OR".
    */
