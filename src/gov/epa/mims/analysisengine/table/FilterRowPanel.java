...
public class FilterRowPanel extends JPanel implements ChildHasChangedListener
{
   ...
   /** The list of all formatters in the table. Used to parse values entered
    *  by the user for numbers and dates. */
   protected Format[] allFormats = null;
   
   /** A hashtable to convert column names to column information.
    * The key is the column name and the data is a Duplex (see end of this
    * class) with Integer index, Class columnClass*/
   protected Hashtable nameToInfoHash = new Hashtable();
   
   /** An array of booleans whose indices match the operation numbers.
    * The value is true if the operation is a String only operation (like
    * starsWith, contains, endsWith...) and false otherwise. */
   ...
   static
   {
      ...
      Arrays.fill(stringOperations, FilterCriteria.STARTS_WITH, FilterCriteria.NOT_ENDS_WITH, true);
   }
   ...
   protected void buildHashtable(Class[] columnClasses)
   {
      ...
      {
         nameToInfoHash.put(allColumnNames[i],
         new Duplex(new Integer(i), columnClasses[i]));
      } // for(i)
   } // buildHashtable()
   
   /**
    * Return the filter criteria based on this GUI.
    * NOTE calling  saveGUIValuesToModel is redundant if you call it before calling
    * this method but it's done as precautionary measure => not a good design!!!
    *
...
//System.out.println("format="+format.getClass());
   ...
   /**
    * For the operations that have to be performed on a String return
    * true.
    */
   ...
   public boolean saveGUIValuesToModel()
   {
      ...
      try
      {
         for (int r = 0; r < rowCount; r++)
         {
            ...
            Integer operation = (Integer)FilterCriteria.symbolToConstantHash.get(operStr);
            operations[r] = operation.intValue();
            
            Duplex duplex = (Duplex)nameToInfoHash.get(names[r]);
            if (duplex == null)
            {
               DefaultUserInteractor.get().notify(this,"Cannot Filter on Hidden Column",
               "In order to filter using the " + names[r] + " column,\n" +
               " you must show that column in the table.", UserInteractor.ERROR);
               return false;
            } // else
            formats[r] = FormattedCellRenderer.getDefaultFormat(
            duplex.columnClass);
            ...
            if (cellValue == null || cellValue.trim().length() == 0)
            {
               ...
               "Empty comparison value", "Please enter a value into the value column.",
            ...
            {
               // We have to check the column type because the DecimalFormat
               // returns a Long for *anything* that can be parsed as an integer.
               // See the DecimalFormat.parseObject() javadoc. :-(
               //WARNING THIS ON THE ASSUMPTION THAT ONLY STRING COLULMNS HAVE NULL FORMATS
               
               if (duplex.columnClass == Integer.class)
               {
                  Number numObject = (Number)formats[r].parseObject(cellValue);
                  values[r] = new Integer(numObject.intValue());
               }
               else if (duplex.columnClass == Double.class)
               {
                  if(cellValue.equals("NaN") || cellValue.equals("Double.NaN"))
               ...
               else if (duplex.columnClass == Boolean.class)
               {
                  ...
                  //System.out.println("bool=" + bool.booleanValue());
         ...
         if (filterCriteria == null)
         {
            ...
            filterCriteria = new FilterCriteria(allColumnNames, /*allFormats,*/ show);
         }
      ...
      {
         DefaultUserInteractor.get().notifyOfException(this,"Invalid filtering value", pe,
         UserInteractor.ERROR);
         return false;
      }
      filterPanel.setHasChanged(false);
