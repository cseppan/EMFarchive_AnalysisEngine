...
public class FileExportGUI extends JDialog
{
   ...
   */
   ...
   public FileExportGUI(TableApp parent, String[] tabNames, int selectedTabIndex)
   {
      ...
      if(tabNames==null)
      {
         new GUIUserInteractor().notify(this,"File Export", "No Tab is Open to "+
"Export", UserInteractor.NOTE);
         return;
      }
...
//      formatPanel.add(originalFormatRB);
      ...
      previewButton.addActionListener(new ActionListener()
      {
         public void actionPerformed(ActionEvent e)
         {
            ...
            JOptionPane.showMessageDialog(FileExportGUI.this, completeFileName,
                  "Example File Name",JOptionPane.INFORMATION_MESSAGE);
         }
      });
...
tableApp.getTablePanelModel(tabNames[index]);
               if(arffRB.isSelected())
                  exportARFF(completeFileName, tablePanelModel);
               else
                  exportFile(completeFileName, delimiter, tablePanelModel);
         ...
         {
            DefaultUserInteractor.get().notifyOfException(this,"Fail to show export status dialog",
                  e,UserInteractor.ERROR);
         }
      }//if(selectedIndices.size()>=1)
   }//exportFiles()

   /** helper method to check whether file is exist and give a message if overwrite
   ...
   private boolean checkForOverwrite(String completerFileName)
   {
      ...
      {
         DefaultUserInteractor.get().notify(this,"WARNING","The file "+
            completerFileName + " is already exists please enter a different name. ",
    ...
    *@param fileName should be absolute fileName
    * @param delimiter String
    * @param data TablePanelModel
    */
