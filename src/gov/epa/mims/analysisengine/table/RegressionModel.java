   ...
   FormatAndIndexInfoIfc
{
   ...
   public SpecialTableModel computeLR(String fileName, boolean genSLR)
      throws Exception
   {
      ...
      if (genSLR)
      {
         ...
         {
            resModel =
               new SLRResultTableModel(SLRDepVariables, SLRIndepVariables,
                  model.getRowCount(), fileName);
         }
         else
         {
            resModel =
               new SLRDetailedResultTableModel(SLRDepVariables,
                  SLRIndepVariables, model.getRowCount(), fileName);
         }
      ...
      else
      {
         ...
         resModel =
            new LRResultTableModel(LRDepVariables, LRIndepVariables,
               model.getRowCount(), fileName);
      ...
      if (genSLR == false)
      {
         ...
         {
            indepInstances =
               Instances.mergeInstances((Instances) indepInstances,
                  (Instances) data.get(indepVars.get(i)));
         }
         ...
         for (int i = 0; i < depVars.size(); i++)
         {
            ...
            {
               indepInstances.setClassIndex(indepVars.indexOf(
                     (String) depVars.get(i)));
            }
            else
            {
               indepInstances =
                  Instances.mergeInstances(indepInstances,
                     (Instances) data.get(depVars.get(i)));
            ...
            if (!indepVars.contains(depVars.get(i)))
            {
               ...
               indepInstances.deleteAttributeAt(indepInstances.numAttributes()
                  - 1);
      ...
      else
      {
         if (conciseReport)
         {
            for (int i = 0; i < depVars.size(); i++)
            {
               ...
               {
                  result.addAll(computeSLR(Instances.mergeInstances(
                           (Instances) data.get(depVars.get(i)),
                           (Instances) data.get(indepVars.get(j))),
                        conciseReport));
               }
         ...
         else
         {
            for (int i = 0; i < depVars.size(); i++)
            {
               ...
               {
                  Vector result =
                     computeSLR(Instances.mergeInstances(
                           (Instances) data.get(depVars.get(i)),
                           (Instances) data.get(indepVars.get(j))),
                        conciseReport);
   ...
   private Hashtable createInstances(Vector depVars, Vector indepVars)
   {
      ...
      for (int k = 0; k < attrs.size(); k++)
      {
         ...
         Instances data1 =
            new Instances(((Attribute) attrs.get(k)).name(), vect,
               model.getRowCount());
         ...
         for (int i = 0; i < model.getRowCount(); i++)
         {
            ...
            {
               value[0] =
                  ((Double) model.getValueAt(i, colIndex)).doubleValue();
            }
            else
            {
               value[0] =
                  (double) ((Integer) model.getValueAt(i, colIndex)).intValue();
            }
   ...
   private Vector computeSLR(Instances data, boolean concise)
      throws Exception
   {
      ...
      Classifier classy =
         (Classifier) Class.forName(
            "weka.classifiers.functions.SimpleLinearRegression").newInstance();
      ...
      if (!concise)
      {
         ...
         result.add(new Double(
               ((weka.classifiers.functions.SimpleLinearRegression) classy)
               .getSlope()));
         result.add(new Double(
               ((weka.classifiers.functions.SimpleLinearRegression) classy)
               .getIntercept()));
      ...
      {
         if (((weka.classifiers.functions.SimpleLinearRegression) classy)
               .getSlope() > 0)
         {
            result.add(new Double(eval.correlationCoefficient()));
         }
   ...
   private Vector computeLR(Instances data) throws Exception
   {
      Classifier classy =
         (Classifier) Class.forName(
            "weka.classifiers.functions.LinearRegression").newInstance();
      ...
      double[] coefficients =
         ((weka.classifiers.functions.LinearRegression) classy).coefficients();
      java.util.Vector result = new java.util.Vector();
      String equation = classy.toString();
      equation =
         equation.substring(equation.indexOf("Linear Regression Model")
