 ...
 * @author Daniel Gatti
 * @version $Id: RowHeaderTable.java,v 1.1 2005/09/19 14:14:04 rhavaldar Exp $
 */
public class RowHeaderTable extends JTable
{
   ...
   /** The formatters used for each column. */
   ...
   {
      super(model);
      //System.out.println("editor name="+getDefaultEditor(Boolean.class).getClass());
      ...
      // Disable this until we get selection working.
   ...
   /**
    * We needed to override this for the case when we change the number of
    * columns in the table. It vaporizes the columns, so we were losing our
    * custom renderers. Overiding this method restores them.
    */
   public void createDefaultColumnsFromModel()
   {
      ...
      if (model != null)
      {
         ...
         TableColumn newColumn = new MultiRowTableColumn(0, new String[]
         {"Row"});
         addColumn(newColumn);
 ...
 */
   ...
   /**
    * Based on the width of the column name and the value in the first row,
    * set the column width. This also fills the Format[] array.
    */
   private void setColumnWidths()
   {
      ...
      if (rend != null)
      {
         Component rendComp = rend.getTableCellRendererComponent(this,
         getValueAt(1, 0), false, false, 1, 0);
         FontMetrics fm = rendComp.getFontMetrics(rendComp.getFont());
         // Get the width of the header and add a 2 spaces on each side.
         int valueWidth =
         fm.stringWidth(getValueAt(getRowCount() - 1, 0).toString()) +
            4 * fm.charWidth('O');
         ...
         if (rend != null)
         {
            ...
            if (rend != null)
            {
               ...
               if (rowCount > 0)
               {
                  ...
                  {
                     rendComp = rend.getTableCellRendererComponent(this,
                        getValueAt(i, c), false, false, i, c);
   ...
   /**
    *
    * Set up the columns to use formatted renderers
    *
    */
   ...
   /**
    * Assign a RowHeaderRenderer to the first column and UnitsCellRenderers
    * to the other columns if we have units.
    */
   private void setUpDataRenderers()
   {
      ...
      MultiRowHeaderTableModel model = (MultiRowHeaderTableModel)getModel();
      MultiRowHeaderRenderer headerRenderer =
      new MultiRowHeaderRenderer(model.getColumnRowHeaders());
   ...
   /**
    * Add the special row headers to the table.
    */
   protected void setUpColumnHeaders()
   {
      ...
      for (int i = 1; i < columnModel.getColumnCount(); i++)
      {
         ...
         new MultiRowHeaderRenderer(model.getColumnHeaders(i-1)));
      }
   } // setUpColumnHeaders()
