...
/**
 * <p>Title: SUSortFilterTablePanel</p>
...
public class SUSortFilterTablePanel extends SortFilterTablePanel
{
   ...
   /** Creates a new instance of SUSortFilterTablePanel */
   public SUSortFilterTablePanel(Component parent,MultiRowHeaderTableModel tableModel)
   {
...
/*
      action = new LoadConfigAction(this);
      popupMenu.add(action);
      JButton LoadConfigButton = toolBar.add(action);
...
*/
   ...
   public void loadConfigFile(File file, boolean loadTableConfig)
   {
      ...
      {
         new GUIUserInteractor().notify(this,"Error Loading Configuration",
               e.getMessage(), UserInteractor.ERROR);
      }
   }

      /**
    * Present an input dialog to the user to get a number. Filter the table to
    * only show the top or bottom N rows. If N is greater than the number of
    * rows, show all rows.
    * @param showTop boolean that is true if we should show the top N rows,
    * false if we should show the bottom N rows.
    * @param isToolBar boolean if true means this action event started from a tool
    * bar click and false means event started from a popup menu.
    */
   public void showNRows(boolean showTop, boolean isToolBar)
   {
      ...
      if (!isToolBar)
      {
         ...
         dialog = new TopNRowsGUI((JFrame) parent, table.getColumnName(selectedColumn), showTop,
               maximumRows);
      } // if(!isToolBar)
      else
      {
      ...
      if (dialog.getResult() == OptionDialog.OK_RESULT)
      {
         ...
         SortCriteria sortCriteria = new SortCriteria(new String[] { columnName},
               new boolean[] { !showTop}, new boolean[] { true});
         ...
         else // showing percentage
         {
            ...
            int cutoffRow = (int) ((double) overallModel.getBaseRowCount()
                  * ((double) numRows / 100.0))- 1;
         ...
         if (filterCriteria == null)
         {
            ...
            TableColumn tableColumn = table.getColumnModel().getColumn(
                  selectedColumn);
            /*
            HasFormatter rend = (HasFormatter) tableColumn.getCellRenderer();
            Format[] formats = { rend.getFormat()};
            */
...
/*formats,*/
       ...
       /*  Format[] allColumnFormats = new Format[table.getColumnCount()-1];
...
*/
   ...
   public void showPlots()
   {
      ...
      {
          new GUIUserInteractor().notify(this,"Show Plots", "Error showing plots "+
            e.getMessage(), UserInteractor.ERROR);
      }
      return;
   }
   ...
   public void showPlotGUI()
   {
      ...
      if (plotGUI.getResult() == OptionDialog.OK_RESULT)
      {
         try
         {
            ...
            /* if(previousConfig.containsKey(plotInfo.getPlotType())) {
             data = (gov.epa.mims.analysisengine.tree.DataSets)previousConfig.get(plotInfo.getPlotType());
             dataSets.add(((Branch)(data.clone())).getChild(0));
             data  =  gov.epa.mims.analysisengine.gui.TreeDialog.showTreeDialog(dataSets, dataSets, null, null);
             }
             else {
             */
   ...
   private String []  setSelectedDataColumns(boolean [] showDataColumns)
   {
      ...
      /* a helper method to set the default option for LineType analysis option
    * depend on the plot type */
   ...
   public void showStatsticsGUI()
   {
      ...
      String fileName = ((TablePanel)(((TableApp)parent).mainTabbedPane.getSelectedComponent())).getFileName();
      String tabName = ((TableApp)parent).mainTabbedPane.getTitleAt(((TableApp)parent).mainTabbedPane.getSelectedIndex());
      ...
      if(statisticsModel.isBasicStatsAnalysis())
      {
         try
         {
            ...
            SpecialTableModel tableModel = new SpecialTableModel(null,colHeader,
               tableData,colType);
            StatisticsModel.basicStats_counter++;
            String basicStatsTabName = statisticsModel.getBasicStatsTabName();
            ((TableApp) parent).insertIntoTabbedPane(tableModel,
basicStatsTabName, basicStatsTabName, null);
         }
         catch(Exception e)
         {
            DefaultUserInteractor.get().notifyOfException(this,"Error", e,
      ...
      if(statisticsModel.isHistogramAnalysis())
      {
         ...
         LabeledDataSetsTableModel dataSetTableModel = new LabeledDataSetsTableModel(
               histogramDataSets);
         dataSetTableModel.setFirstColumnName("Bins");
         ((TableApp) parent).insertIntoTabbedPane(dataSetTableModel, null,
               histogramTabName, null);
         }
      }//if(statisticsModel.isHistogramAnalysis())
      ...
      if(statisticsModel.isCorrelationAnalysis())
      {
         try
         {
            ...
            SpecialTableModel sModel = model.computeLR(fileName, true);
            ((TableApp)parent).insertIntoTabbedPane(sModel, model.getTabName(true),
               model.getTabName(true), FileImportGUI.COMMA_DELIMITED_FILE);
         }
         catch(Exception ie)
         {
        ...
        try
         {
           ...
           RegressionModel model = statisticsModel.getRegressionModel();
           SpecialTableModel sModel = model.computeLR(fileName, false);
           ((TableApp)parent).insertIntoTabbedPane(sModel,
model.getTabName(false), model.getTabName(false), FileImportGUI.COMMA_DELIMITED_FILE);
         }
         catch(Exception ie)
         {
             new GUIUserInteractor().notify(this,"Error", "Unable to generate Linear "+
"Regression data: "+ie.getMessage(), UserInteractor.ERROR);
             return;
         }  
      }
   ...
   class ShowTopOrBottomValuesAction extends AbstractAction
   {
      ...
      public ShowTopOrBottomValuesAction(SUSortFilterTablePanel parent,
         boolean isTop, boolean isToolBar)
      {
         super("Show " + ((isTop) ? "Largest" : "Smallest") + " N rows...",
               (isTop) ? topNRowsIcon : bottomNRowsIcon);
         this.isTop = isTop;
