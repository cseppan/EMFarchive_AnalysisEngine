...
import gov.epa.mims.analysisengine.table.DoubleTableFromFile;
 ...
 * @author  parthee
 * @version $Id: FileImportGUI.java,v 1.1 2006/10/30 17:26:13 parthee Exp $
 */
...
public class FileImportGUI extends JDialog
{
   ...
   /** whether to consider multiple occurences of the
    *  delimiter character as one or not
    */
...
/*   public static final String TRIM_RESULTS_FILE = "TRIM Results File";
   public static final String TRIM_SENSITIVITY_FILE = "TRIM Sensitivity File";
   public static final String GENERIC_FILE = "Generic File";
   public static final String MONTE_CARLO_FILE = "Monte Carlo File";
 ...
 */
   ...
   private final String [] ALL_FILE_TYPES =
   {
      COMMA_DELIMITED_FILE,
      COSU_FILE,
      GENERIC_FILE,
      DAVE_OUTPUT_FILE,
      FIXED_WIDTH_FILE,
    ...
    * fileType can be
    * FileImportGUI.TRIM_RESULTS_FILE
    * FileImportGUI.TRIM_SENSITIVITY_FILE
    * FileImportGUI.DAVE_OUTPUT_FILE
    * FileImportGUI.MONTE_CARLO_FILE
    * FileImportGUI.COSU_FILE
    * FileImportGUI.COMMA_DELIMITED_FILE
    * FileImportGUI.SMOKE_REPORT_FILE
    * FileImportGUI.GENERIC_FILE
    */
   ...
   private void initialize()
   {
      ...
      startTabIndexPanel= new IntegerValuePanel(
         "Name the tab based on characters at positions ",
         false,1, Integer.MAX_VALUE);
      tabNameConfigPanel.add(startTabIndexPanel);
      endTabIndexPanel= new IntegerValuePanel("through",
         false,1, Integer.MAX_VALUE);
      ...
      previewButton.addActionListener(new ActionListener()
      {
         public void actionPerformed(ActionEvent e)
         {
            if(table.getRowCount() >0)
            {
               ...
               fileNameWithExt.substring(0,wholeLength-4);
               int endIndex = -1;
               int startIndex =-1;
               ...
               String message = "The truncated tab name of the first file in"+
                  " the table = " +
               fileNameWOExt.substring(startIndex-1,endIndex);
               JOptionPane.showMessageDialog(FileImportGUI.this,message,
                  "Example Tab Name", JOptionPane.INFORMATION_MESSAGE);
            }// if(table.getRowCount() >0)
         }
      });
      ...
      PlainDocument plainDoc = new PlainDocument()
      {
         ...
         {
            if (offs != 0) throw new BadLocationException( "You should only enter "+
               "one character", offs);
            else if (str.length() > 1) throw new BadLocationException("You should "+
               "only enter one character", str.length());
...
/*      noOfcolumnHeaderPanel = new IntegerValuePanel("Number of column header"+
      " rows ", false,0, Integer.MAX_VALUE);
      noOfcolumnHeaderPanel.setValue(1);
      addFileInfoPanel.add(noOfcolumnHeaderPanel);
 */
...
//      mainPanel.add(fileNamePanel);
      ...
      fileTypeComboBox.addActionListener(new ActionListener()
      {
         public void actionPerformed(ActionEvent e)
         {
            ...
            /*if(table.getRowCount() > 0)
             {
                int no = DefaultUserInteractor.get().selectOption(
                  "Warning","You will loose the all the rows, if you change"+
                  " the file type.",UserInteractor.OK_CANCEL,
                  UserInteractor.CANCEL);
                if(no == UserInteractor.YES)
                {
                  removeAllRows();
                }
             }// if(table.getRowCount() > 0)
...
/*      JButton previewButton = new JButton("Preview");
      ...
      previewButton.setToolTipText("Preview the file selected in the file"+
 ...
 */    //editPanel.setLayout(new BoxLayout(editPanel,BoxLayout.X_AXIS));
      ...
      
      {
         ...
         
         {
            ...
            if(rows.length==0 && table.getRowCount()>0)
               ...
               "Select a file in the above table to preview the contents",
               ...
               if(rows.length>1)
                  ...
                  "Select only one file in the above table "+
                  
                  "to preview the file contents", UserInteractor.NOTE);
                     ...
                     
                  {
                     ...
                     try
                     {
                        ...
                        TextDialog dialog = new TextDialog(parent, "Preview "+
                     ...
                     
                     {
                        ...
                        System.out.println("Error bringing up the file preview "+
    ...
    *
    * @param selectedType String selected file type
    *
    * @param delimiter for the file
    *
   ...
   public static SpecialTableModel createAModel(String fileName,
   String selectedFileType, String delimiter, int noOfColNameRows)
   throws Exception
   {
      if(selectedFileType.equals(GENERIC_FILE))
      {
         ...
         FileParser reader = new FileParser(fileName, delimiter,
            noOfColNameRows, multipleOccurences);
         return createSpecialTableModel(reader);
      }//if(selectedFileType.equals(GENERIC_FILE))
      
      else if (selectedFileType.equals(COMMA_DELIMITED_FILE) ||
      selectedFileType.equals(COSU_FILE))
      {
         if(selectedFileType.equals(COSU_FILE))
            noOfColNameRows = 1;
     ...
     /*   kthanga - commented out because COSUAdapter is not working properly.
      *   Currently using FileParser for processing this file.
         else if (selectedFileType.equals(COSU_FILE))
         {
               //cosu assumes a "," delimiter
               COSUAdapter adapter = new COSUAdapter(fileName);
               String [][] columnHeader = adapter.getColumnHeader();
               ArrayList tableData = adapter.getFileData();
               return new SpecialTableModel(null,columnHeader,tableData,
                      adapter.getColumnClass());
         }//else if (selectedFileType.equals(this.COSU_FILE)) */
      ...
      {
         TRIMSensitivityFileReader reader =
         new TRIMSensitivityFileReader(fileName, multipleOccurences);
         return createSpecialTableModel(reader);
      }//else if(selectedFileType.equals(TRIM_SENSITIVITY_FILE))
      else if(selectedFileType.equals(DAVE_OUTPUT_FILE))
      {
      ...
      {
         SMKReportFileReader reader =
         new SMKReportFileReader(fileName,delimiter, multipleOccurences);
         return createSpecialTableModel(reader);
      }//else if(selectedFileType.equals(SMOKE_REPORT_FILE))
      else if(selectedFileType.equals(ARFF_FILE))
      {
         ARFFReader reader =
         new ARFFReader(fileName);
         return createSpecialTableModel(reader);
      }
      return null;
   }
   ...
   */   
   private static SpecialTableModel createSpecialTableModel(FileParser reader)
   {
      ...
      return new SpecialTableModel(fileHeader,rowHeaderData,colHeader,
         tableData,fileFooter, columnTypes);
   }
   
   /** a helper method to convert a array within a ArrayList to a two
    * dimensional array.
    */
