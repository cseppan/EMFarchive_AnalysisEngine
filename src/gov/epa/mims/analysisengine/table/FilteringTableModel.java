 ...
 * model that contains all of the data. Based on the filter criteria, it
 * creates an array of indices that map to values in the underlying model. For
...
implements TableModelListener
{
   ...
   /** A hashtable that maps column names to thier index in *this* table. (not
    * in the underlying table. This is used to lookup column indices by the
    * SortingModel. */
   protected Hashtable nameToIndexHash = new Hashtable();
    ...
    * @param criteria FilterCriteria with column filtering data.
    */
    ...
    * @param col int that it the column.
    * @return Class that is the class for teh requested column.
    */
   public Class getColumnClass(int col)
   {
      //System.out.println("Filter: getColumnClass("+colMap[col]+") = " + underlyingModel.getValueAt(0, colMap[col]).getClass());
      Class retval = String.class;
      try
      {
         retval = underlyingModel.getColumnClass(colMap[col]);
      } catch(Exception e)
      { //in case of absence of getColumnClass()
   ...
   /**
    * Return the value at the given row and column for this model. This
    * mean that we take the index from the filtrMap, which maps values from
    * the GUI row and column to the underlyingModel row and column.
    * @param rowIndex
    * @param columnIndex
    * @return
    */
   ...
   /**
    * Clear and reset the row and column maps to just pass data through
    * unchanged.
