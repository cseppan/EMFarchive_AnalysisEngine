 ...
 * <p>Title:PercentileGUI.java </p>

 * <p>Description: A gui for the user to specify the bin parameters
 ...
 * @version $Id: PercentileGUI.java,v 1.2 2005/09/19 14:50:03 rhavaldar Exp $
...

{
   ...
   /** Creates a new instance of PercentileGUI */
...

   {
      ...
      customOptionPanel.setBorder(BorderFactory.createTitledBorder(

         lineBorder,"Frequent Options"));
...
//      minMaxPanel.add(minValuePanel,BorderLayout.NORTH);

//      minMaxPanel.add(maxValuePanel);

//      JPanel restPanel = new JPanel();

//      restPanel.setLayout(new BorderLayout());

//      restPanel.add(stepSizeValuePanel,BorderLayout.NORTH);

//      restPanel.add(addButtonPanel);

      JPanel addPercentilePanel = new JPanel();

      addPercentilePanel.setBorder(BorderFactory.createTitledBorder(lineBorder,
      ...
      contForPercentPanel.setBorder(BorderFactory.createTitledBorder(

         BorderFactory.createLineBorder(Color.black),"Percentiles"));

      JPanel addAndTablePanel = new JPanel();

      //addAndTablePanel.setLayout(new BoxLayout(addAndTablePanel,BoxLayout.X_AXIS));
      ...
      //mainPanel.add(addPercentilePanel);

      //mainPanel.add(contForPercentPanel);
...
//      mainPanel.add(createOKCancelPanel(),BorderLayout.SOUTH);
   ...
   /** a helper method to add percentile to the tables
...

   {
...

      {
         ...
         DefaultUserInteractor.get().notify(this,"Input Error","Please input a "+

            "value for lower percentile",UserInteractor.ERROR);
...

      {
         ...
         DefaultUserInteractor.get().notify(this,"Input Error","Please input a "+
            ...
            "value for upper percentile",UserInteractor.ERROR);
...

      {
         ...
         DefaultUserInteractor.get().notify(this,"Input Error","Lower percentile value "+

            minPercentile + " is greater than upper percentile value "+ maxPercentile,
...

      {
         ...
         DefaultUserInteractor.get().notify(this,"Input Error","Please input a "+
            ...
            "value for step size",UserInteractor.ERROR);
...

      {
         ...
         DefaultUserInteractor.get().notify(this,"Input Error","Please input a "+
            ...
            "step size smaller than difference between upper percentile value " +
  ...
  /** a method to save the gui data to the model
...

   {
...

      {
         ...
         DefaultUserInteractor.get().notify(this,"Tab Name",

            "Please specify the tab name", UserInteractor.ERROR);
...

      {
         ...
         DefaultUserInteractor.get().notify(this,"Duplicate values",
...

      {
         ...
         DefaultUserInteractor.get().notify(this,"No Percentiles",

            "Specify the percentile values", UserInteractor.ERROR);
...

      {
         ...
         DefaultUserInteractor.get().notify(this,"Not Sorted",
   ...
   /** return the percentile values

    * @return double[] values enter in the percentilePanel
