...
/**
 * <p>Description: A panel that shows formatting doubles.  It includes scientific
 * notation, percent, dollars and a sample label to show the user what
 * their format will look like. </p>
   ...
   implements ActionListener
{
   ...
   protected JRadioButton standardButton =
      new JRadioButton(SignificantDigitsFormat.STANDARD_FORMAT, false);

   /** A radio button that allows for scientific format. */
   protected JRadioButton scientificButton =
      new JRadioButton(SignificantDigitsFormat.SCIENTIFIC_FORMAT, true);

   /** Radio button for percentage format */
   protected JRadioButton percentButton = new JRadioButton(
         SignificantDigitsFormat.PERCENTAGE_FORMAT, false);

   /** radio button for dollars i.e. currency */
   protected JRadioButton dollarsButton = new JRadioButton(
         SignificantDigitsFormat.CURRENCY_FORMAT, false);
    ...
    * @param availableFormats String[] with pre-defined formats. (Could be
    *    null. If it is, we supply formats)
    ...
    * @param showPercent boolean that is true if we should show the percent
    *    check box.
    * @param showDollars boolean that is true if we should show the dollars
    *    check box.
    */
   public DoubleFormatPanel(
                            Format format, String[] availableFormats,
                            boolean allowUserValue, boolean showPercent,
                            boolean showDollars)
   {
      this.format = format;

      this.setBorder(BorderFactory.createCompoundBorder(
         BorderFactory.createTitledBorder("Numeric Format Options"),
         BorderFactory.createEmptyBorder(5,5,5,5)));
      decimalSpinner = new NumericSpinner("Decimal Places:", 3.0,
            new DecimalFormat("0"), 1.0, 0.0, 10.0);
      ...
      sigDigitsSpinner = new NumericSpinner("Significant Digits:", 4.0,
            new DecimalFormat("0"), 1.0, 1.0, 10.0);
      ...
      formatField.setToolTipText(
         "Edit this field only if you understand the formatting string.");
   ...
   protected Format createNewFormat(String formatString)
   {
      ...
      SignificantDigitsFormat newFormat = new SignificantDigitsFormat(
            formatString);
   ...
   protected void setSampleLabel()
   {
      ...
      {
         String number = fieldFormat.format(sampleObject);
   ...
   /**
    * When something about the format has changed, update the format field and
    * then change the sample label.
    * PPP - The format pattern is later picked up from this field.
    */
