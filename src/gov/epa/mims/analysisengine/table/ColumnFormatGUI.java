...
/**
 * <p>Description: A GUI that can edit the format for either a single column
 * or multiple columns. </p>
 * <p>Copyright: Copyright (c) 2003</p>
 * <p>Company: UNC - CEP </p>
 * @author Daniel Gatti
 * @version $Id: ColumnFormatGUI.java,v 1.2 2005/09/19 14:50:03 rhavaldar Exp $
 */
public class ColumnFormatGUI extends OptionDialog implements TableModelListener
{
   ...
   /** The column headers. This in only used (and hence non-null) in MULTIPLE
    * mode.*/
   ...
   /** The column selection GUI for the case where we are editing multiple
    * columns. This in only used (and hence non-null) in MULTIPLE mode.*/
    ...
    *
    * @param columnName String that is the name of the column being edited.
    * @param columnFormatInfo ColumnFormatInfo that contains the formatting
    *    information for the column being edited.
    */
    ...
    *
    * @param columnHeaders String[][] that contains the column headers for all
    *    columns.
    * @param formats Format[] that contains the Format for each column in the
    *    order passed in the columnHeaders array.
    * @param columnFormatInfo ColumnFormatInfo that contains the formatting
    *    information for the columns being edited.
    */
   public ColumnFormatGUI(JFrame parent, String[][] columnHeaders, Format[] formats,
    ...
    * @param columnHeaders String[][] that contains the column headers for all
    *    columns.
    * @param selColumns boolean [] true for selected and false for unselected
    * @param formats Format[] that contains the Format for each column in the
    *    order passed in the columnHeaders array.
    * @param columnFormatInfo ColumnFormatInfo that contains the formatting
    *    information for the columns being edited.
    ...
    */
    protected void initialize()
    {
       ...
       else // MULTIPLE
       {
          ...
          selectionPanel = new ColumnSelectionPanel(columnHeaders, selColumns,
             "Format", "Ignore");
    ...
    * @return getSelectedColumns() boolean[] with a true for each selected
    * column. This is in the same order as the column names passed in.
    *
   ...
   /**
    * Respond to new columns that have been checked or unchecked in the column
    * selection panel. When this happens, check the formats for the checked
    * columns. If they are all of the same type (Number or Date), then
    * show a formatting panel with
    */
...
//System.out.println("tableChanged()");
      ...
      for (int c = 0; c < checkedColumns.length; c++)
      {
         if (checkedColumns[c])
         {
//System.out.println(c + " checked : " + formats[c].getClass());
            if (lastFormat != null)
            {
