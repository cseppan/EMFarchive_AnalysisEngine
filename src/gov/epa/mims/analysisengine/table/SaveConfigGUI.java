...
/**
 * SaveConfigGUI.java - Dialog to save plots and table configuration
 * configured during the current session since the beginning of the 
 * opening of the table in the application. 
 * This dialog pops up when user clicks the Load Configuration from 
 * TablePanel icons or from the file menu.
 * It has several features:
 * View - to view the configuration in the current table context as 
 * a plot
 * Edit - a Plot Options panel opens up displaying the current 
 * configuration for the user to edit the selected configuration
 * Delete - deletes the selected configuration from the currrent 
 * configuration
 * Select - enables the Save As options on the selected configurations
 * Clear - clears the Save As option on the selected configurations
 * Save As - save the current configuration based on "Save?" selection
...
public class SaveConfigGUI extends javax.swing.JDialog {
   ...
   /** Textfield displaying the selected output file or for inputting
    * the filename */
...
//         chooseConfigTable.getTableHeader().addMouseListener(new MouseAdapter() 
//         {
//            public void mouseClicked(MouseEvent evt)
//            {
//               bTablHdrMouseClicked();
//            }
//         });      
      ...
      FlowLayout ConfigTableModifierLayout = new FlowLayout(FlowLayout.CENTER,
               3, 4);

         ConfigTableModifierPanel.setLayout(ConfigTableModifierLayout);
         ConfigTableModifierPanel.setPreferredSize(
               new java.awt.Dimension(400, 40));
   ...
   protected void bViewMouseClicked(MouseEvent evt) {
      ...
      if(indexs.length < 1)  {
         if(chooseConfigTable.getRowCount()==0) return;
      ...
      try {
           ...
           {
               new GUIUserInteractor().notify(this,"View "+configName,
                   configName+" reflects the current state of the table",
                   UserInteractor.NOTE);
               continue;
          }
         ...
         gui.notify(this,"Error with " + model.getValueAt(indexs[i], 0), e.getMessage(),
               UserInteractor.ERROR);
         int result = gui.selectOption(this,"Delete Faulty Configuration?",
               "Delete FaultyConfiguration?", GUIUserInteractor.YES_NO,
               GUIUserInteractor.NO);
   ...
   protected void bEditMouseClicked(MouseEvent evt) {
      ...
      if(indexs.length > 1) 
      {
         new GUIUserInteractor().notify(this,"Note", "Choose only one configuration " + 
            "at a time to edit", UserInteractor.ERROR);
         return;
      }

      if(indexs.length==0) 
      {
         new GUIUserInteractor().notify(this,"Note", "No selected configuration to edit",
            UserInteractor.NOTE);
          return;
      }
   ...
   protected void bDeleteMouseClicked(MouseEvent evt) {
      ...
      while (k-- > 0) {
               ...
               configName.equalsIgnoreCase(AnalysisConfiguration.TABLE_FORMAT))
         {
            ...
            " this table later in this session.", UserInteractor.YES_NO,
            UserInteractor.NO);
...
//   }
   ...
   protected void bOkMouseClicked(MouseEvent evt) {
      ...
      if (fileName == null || fileName.length() == 0) {
         new GUIUserInteractor().notify(this,"Error Saving Configuration",
               "No Configuration File specified", UserInteractor.ERROR);
         return;
      }
      try {
   ...
   /** MyCellEditorListener - to propogate changes to the ConfigTable
    *  to the AnalysisConfiguration object in the table
    */
   ...
   class MyCellEditorListener implements CellEditorListener {
      ...
      public void editingStopped(ChangeEvent e) {
         ...
         try {
            model.renameConfig(row,
                  (String) ((TableCellEditor) (e.getSource())).getCellEditorValue());
         } catch (Exception ex) {
            new GUIUserInteractor().notify(SaveConfigGUI.this,"Error", ex.getMessage(),
                  UserInteractor.ERROR);
            chooseConfigTable.getModel().setValueAt(model.getConfigName(row), row,
                  0);
         }
      }
