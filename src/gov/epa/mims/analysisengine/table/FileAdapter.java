...
/**
 * <p>Title:FileAdapter.java </p>
 * <p>Description: A class to create adapter which will have information about 
 * the file name, file type, delimiter, no of column header rows for the file </p>
...
public class FileAdapter
{
   ...
   int plotFmt = SaveToDialog.PDF;
    ...
    /** hashmap to store file type  information */
   ...
   static
   {
      ...
      fileDelimiter.put(FileImportGUI.SMOKE_REPORT_FILE, 
         SMKReportFileReader.DELIMITER);
      fileDelimiter.put(FileImportGUI.TRIM_RESULTS_FILE, 
         TRIMResultFileReader.DELIMITER);
      fileDelimiter.put(FileImportGUI.TRIM_SENSITIVITY_FILE,
         TRIMSensitivityFileReader.DELIMITER);
      fileDelimiter.put(FileImportGUI.MONTE_CARLO_FILE,
         MonteCarloFileReader.DELIMITER);
      fileDelimiter.put(FileImportGUI.COSU_FILE, 
         COSUAdapter.DELIMITER);
      ...
      fileColumnHeaderInfo.put(FileImportGUI.SMOKE_REPORT_FILE, 
         new Integer(SMKReportFileReader.NO_OF_COLUMN_HEADER_ROWS));
      fileColumnHeaderInfo.put(FileImportGUI.TRIM_RESULTS_FILE, 
         new Integer(TRIMResultFileReader.NO_OF_COLUMN_HEADER_ROWS));
      fileColumnHeaderInfo.put(FileImportGUI.TRIM_SENSITIVITY_FILE, 
         new Integer(TRIMSensitivityFileReader.NO_OF_COLUMN_HEADER_ROWS));
      fileColumnHeaderInfo.put(FileImportGUI.MONTE_CARLO_FILE,
         new Integer(MonteCarloFileReader.NO_OF_COLUMN_HEADER_ROWS));
      fileColumnHeaderInfo.put(FileImportGUI.COSU_FILE,
         new Integer(COSUAdapter.NO_OF_COLUMN_HEADER_ROWS));
      fileColumnHeaderInfo.put(FileImportGUI.DAVE_OUTPUT_FILE,
         new Integer(DAVEFileReader.NO_OF_COLUMN_HEADER_ROWS));
   ...
   * @throws Exception if and when arg. are spefied inappropriately
   */
...
//System.out.println(args[i]);
         ...
         else if (currentArg.equalsIgnoreCase("-delimiter"))
         {
            ...
            if(length >=2)
            {
               throw new Exception("Delimiter should have one character but has "+
                  length + " characters ");
            }//ifs
         }//else if
         else if (currentArg.equalsIgnoreCase("-hRows"))
         {
            ...
            {
               throw new NumberFormatException("Could not convert no of column header "
                  + args[i] + " to an integer");
            }//catch
         }//else if
         else if (currentArg.equalsIgnoreCase("-startPos"))
         {
            ...
            {
               throw new NumberFormatException("Could not convert startPos " +
                  args[i] + " to an integer");
            }//catch
         }//else if
         else if (currentArg.equalsIgnoreCase("-endPos"))
         {
            ...
            {
               throw new NumberFormatException("Could not convert endPos " +
                  args[i] + " to an integer");
            }//catch
         }//else if
         ...
         else if(currentArg.equalsIgnoreCase("-ConfigFile"))
         {
            ...
            {
               System.out.println("Configuration file "+ configFile+"  does not"+
                  " exist\n");
               configFile = null;
            }
         }
   ...
   public static void printUsage()
   {
      ...
      System.out.println("<type>                  = One of these types: "+
      FileImportGUI.COMMA_DELIMITED_FILE + ", "+
      FileImportGUI.TAB_DELIMITED_FILE + ", "+
      ...
      FileImportGUI.FIXED_WIDTH_FILE + ", " + 
      FileImportGUI.ARFF_FILE + ", " + 
      FileImportGUI.DAVE_OUTPUT_FILE );
      System.out.println("<delimiter>             = A single character; Required only for "
         + "DAVE Output files and Custom delimited files, Optional for SMOKE "+
"Report File\n");
      System.out.println("<noOfRows>              = An integer value indicates the number of column "
         ...
         + "Custom delimited files, Comma Separated Files and Tab Delimited "+
" Files\n");
      System.out.println("<path>                  = complete path of the "+
"Configuration File\n");
      System.out.println("-ignoreTableConfig    specify when the plots are to "+
         "plotted with entire table data without any filter/sort/format "+
         "applied\n");
      ...
      System.out.println("<directory>             = destination directory to which the plots"+
         " are to be saved\n");
   }//printUsage()
   ...
   private void checkDelimiter(String delimiter) throws Exception
   {
      ...
      {
         throw new Exception("For the '" + fileType + 
   ...
   {
      Integer tempNoOfColumnHeaderRows =(Integer)FileAdapter.fileColumnHeaderInfo.get(
         fileType);
      ...
      {
         throw new Exception("The file type '"+ fileType+
