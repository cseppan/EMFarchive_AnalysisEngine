...
/**
 * <p>Description: A panel that allows the user to select column names.
 *  based on some filtering criteria. </p>
 * <p>Copyright: Copyright (c) 2003</p>
 * <p>Company: UNC - CEP</p>
 * @author Daniel Gatti
 * @version $Id: ColumnSelectionPanel.java,v 1.2 2005/09/19 14:50:03 rhavaldar Exp $
 */
public class ColumnSelectionPanel extends JPanel implements ListSelectionListener
{
   ...
   /** FilterCriteria for choosing columns. Note that this is *NOT* the same
    * as the FilterCriteria held by the main table. This one is only for
    * selecting columns. */
    ...
    *
    * @param columnHeaders String[][] that is the list of headers for each
    *    column.
    */
    ...
    * @param columnHeaders String[][] that is the list of headers for each
    *    column.
    * @param selected boolean[] where an element is true if the column is
    *    selected.
    */
   public ColumnSelectionPanel(String[][] columnHeaders,
                               boolean[] selected)
    ...
    * @param selected boolean[] where an element is true if the column is
    *    selected.
    * @param checkString String to place on the "check" button.
    * @param uncheckString String to place on the "uncheck" button.
    */
   public ColumnSelectionPanel(String[][] columnHeaders,
                               boolean[] selected, String checkString,
   ...
   /**
    * Allow another class to listen when the table values are checked or
    * unchecked.
    */
   ...
   /**
    * Take the filter that is shown and select rows in the column name
    * table based on the filter.
    * @param check
    */
   ...
   //   Format[] formats     = new Format[columnNames.length];
      ...
      for (int i = 0; i < tableData.length; i++)
      {
         ...
         Integer intObj =
               (Integer)FilterCriteria.symbolToConstantHash.get(tableData[i][1]);
  ...
  //       formats[i] = FormattedCellRenderer.nullFormatter;
    ...
    * @param check boolean that is true if we should check the selected values
    *    and false if we should uncheck them.
    */
    ...
    *    selected.
    */
   protected void initialize(String[][] columnHeaders,
                               boolean[] selected, String checkString,
                               String uncheckString)
   {
      // Table
      ...
      // Filter Panel
      filterPanel = new FilterPanel(true, null, new String[] {"Column Name"},
                                    FilterCriteria.OPERATION_STRINGS, 0,
   ...
   /**
    * Return the checked columns.
    * <b>NOTE: even if the user has sorted the column names, we will still
    * return this array as though they had not sorted. </b>
    *
   ...
   /**
    * Reverse the current selection of columns in the table.
...
//      Format[] formats = new Format[tableData.length];
...
//         formats[r] = FormattedCellRenderer.nullFormatter;
      } // for(r)

      if (localCriteria == null)
      {
         localCriteria = new FilterCriteria(cols,
               ops, values,/* formats, */cols, filterPanel.getUseAnd());
      } // if (localCriteria == null)
      else
      {
         localCriteria.setRowCriteria(cols,
               ops, values, /*formats, */ filterPanel.getUseAnd());
      }
   } // saveGUIValuesToModel()


   /**
    * Update the selection label when the selection in the table changes.
    *
    * For the ListSelectionListener interface.
