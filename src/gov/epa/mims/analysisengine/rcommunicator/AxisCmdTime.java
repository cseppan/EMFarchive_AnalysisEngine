 ...
 * @author $author$
 ********************************************************/
public class AxisCmdTime extends AxisCmdTime2
{
   /**
    * Creates a new AxisCmdTime object.
    *
    * @param side DOCUMENT_ME
    * @param options DOCUMENT_ME
    ********************************************************/
   public AxisCmdTime(int side, AxisTime options)
   {
      super(side, options);
      ...
      if (options.getGridEnable())
      {
         ...
         long increment = (options.getGridIncrement()!=null)?(options.getGridIncrement().longValue()):(-1);
