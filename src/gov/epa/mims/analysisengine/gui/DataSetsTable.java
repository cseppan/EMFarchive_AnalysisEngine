  ...
  extends JTable
{
   ...
   protected void setDataSetsForPlot(Plot plot)
   throws Exception
   {
     // TBD: come back and make the auto selection work again
     /*dataSets.toArray(datasetArray);
     for (int i = 0; i < dataSets.size(); i++)
     {
        // don't fill in all the data sets if the max is not N
        int max = dataSetInfos[i].getMaxNumber();
        if ((datasetArray[i].getKeys().size() == 0) && (max == -1))
        {
          // if nothing is selected, automatically select all the data sets in
          // the data sets adapter
          DataSets ds = (DataSets)dataSets.get(i);
          Vector keyVect = dataSetsAdapter.getDataSetKeys(
             null, this.getParent());
          // TBD: if only one data set is required, don't add them all
          for (int j = 0; j < keyVect.size(); j++)
          {
            Object key = keyVect.get(j);
            ds.add(dataSetsAdapter.getDataSet(key), key);
          }
          this.repaint();
        }
     }
   ...
   public class DataSetsTableModel
         extends AbstractTableModel
   {
      ...
      public int getRowCount() { return dataSetInfos.length; }
   ...
   public class DataSetTableCellEditor extends JButton implements TableCellEditor
   {
      ...
      void edit()
      {
         ...
         Vector selected = DataSetSelector.pickDataSets(
            dataSetsAdapter, currentSelection, dataSetInfos[index]);
