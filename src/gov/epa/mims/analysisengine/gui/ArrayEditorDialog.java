...
/**
 * A helper Dialog to edit arrays of values.. Presently edits double[], Color[],
 ...
 *****************************************************/
  ...
  * @param doubles the double[] to be edited
  */
  public ArrayEditorDialog(double[] doubles, String label,
   ...
   * @param colors the Color[] to be edited
   */
    ...
    if (type.equals(DOUBLES))
    {
      ...
      addButton.addActionListener(new ActionListener(){
        public void actionPerformed(ActionEvent ae)
        {
          ...
          try
          {
            String enter = (new GUIUserInteractor()).getString(
                "Enter a floating point value", "Add new value",
                "0.0");
            if (enter == null) return;
            value = new Double(enter);
            if (value.doubleValue() < lowBound || value.doubleValue() > upBound)
              throw new Exception("Value should be between " + lowBound + " and "
              + upBound);
...
//            e.printStackTrace();
    ...
    else if (type.equals(COLORS))
    {
      ...
      editListener = new ActionListener()
      {
        public void actionPerformed(ActionEvent ae)
        {
          ...
          {
            int index = list.getSelectedIndex();
            value = JColorChooser.showDialog(null,"Choose Color",
                (Color) objectList.get(index));
            if (value == null) return;
            objectList.remove(index);
            objectList.add(index, value);
            list.setListData(objectList.toArray());
          ...
          try
          {
            if (availStringChoices != null)
              value = (String) JOptionPane.showInputDialog(null,
                ...
                availStringChoices, availStringChoices[0]);
            else
        ...
        public void actionPerformed(ActionEvent ae)
        {
          ...
          try
          {
            ...
            value = (String) JOptionPane.showInputDialog(null,
                "Select one for the available options", "Choose one",
                JOptionPane.INFORMATION_MESSAGE, null,
                availStringChoices, list.getSelectedValue());
            if (value == null) return;
            objectList.remove(index);
            objectList.add(index, value);
            list.setListData(objectList.toArray());
   ...
   * @param doubles double[] to edit
   * @return double[] the edited double array
   */
  public static double[] editDoubles(double[] doubles)
  {
    ArrayEditorDialog arrayEditor = new ArrayEditorDialog(doubles, "", Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY);
  ...
  /**
   * static method to edit a set of strings in a dialog
   * these are chosen from a available set of strings
   * @param origChoices the original set of choices that the user wants to edit
   * @param availChoices the available set of values
...
/*
         ...
         {
           ((JLabel) rend).setText("");
           rend.setForeground((Color) value);
           rend.setBackground((Color) value);
           ((JLabel) rend).setMinimumSize(new Dimension(0, 20));
           ((JLabel) rend).setMaximumSize(new Dimension(0, 20));
           //((JLabel) rend).setSize();
         }
         System.out.print("min " + ((JLabel) rend).getMinimumSize().getHeight());
         System.out.println(" max " + ((JLabel) rend).getMaximumSize().getHeight());
         return rend;
*/
       }
   }
