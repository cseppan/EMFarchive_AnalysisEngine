...
/**
 * A dialog for presenting options.  Each type of option stores data in a
 * data object, and that info is presented in the GUI.
 * Derived classes should set the result appropriately when they are closed
 * to OK_RESULT or CANCEL_RESULT.   The info in getInfoString will be shown
 * in the OptionsTable.
 *
    ...
    extends MDialog
{
   ...
   /** If true, then do not continue to close the GUI becuase there is
    * an error that the user needs to correct. */
   protected boolean shouldContinueClosing = true;

   /** The type of plot that is being edited. This is used to customize
    * the GUI's for each plot. Use the plot constants in AnalysisEngineConstants.*/
...
*/
   ...
   protected JPanel getButtonPanel()
   {
    ...
    JPanel buttonPanel = new OKCancelPanel(
      okListener, cancelListener, getRootPane());
    return buttonPanel;
   }
   
   public JPanel getButtonPanel(boolean helpButton, 
      ActionListener helpListener)
   {
      if(helpButton)
      {
         ActionListener okListener = getOKListener();
         ActionListener cancelListener = getCancelListener();
         JPanel buttonPanel = new OKCancelHelpPanel(
         okListener, cancelListener,helpListener,getRootPane());
         return buttonPanel;
      }
      else
      {
   ...
   private ActionListener getOKListener()
   {
      ActionListener okListener =  new ActionListener()
      {
         public void actionPerformed(ActionEvent ae)
         {
           ...
           {
             DefaultUserInteractor.get().notifyOfException(OptionDialog.this,"Error Setting "
                 + " Plot Properties", e, UserInteractor.ERROR);
           }//catch
         }//actionPerformed
      };
