 ...
 *****************************************************/
  ...
  public DataSetSelector(DataSetsAdapter dataSetsAdapter,
     Vector initialSelection, DataSetInfo dsInfo)
  {
  ...
  private void initializeDataSets(Vector initialSelection)
  {
    ...
    Iterator allDSIt = dataSetsAdapter.getDataSetKeys(dataSetType,
       this.getContentPane()).iterator();
      ...
      while (selDSIt.hasNext())
      {
        ...
        if (existingDataSet == null)
        {
          throw new IllegalArgumentException(
              "Data set "+dswk.getName()+" not found in available data sets");
        }
        else if (existingDataSet != dswk.dataSet)
        {
          throw new IllegalArgumentException(
              "Initial data set "+existingDataSet.toString()+
  ...
  /*
    ...
    {
      // select all data sets in initial selection
      Iterator selDSIt = initialSelection.getDataSetKeys(dataSetType,
         this.getContentPane()).iterator();
      while (selDSIt.hasNext())
      {

        Object key = selDSIt.next();
        DataSetIfc dataSet = initialSelection.getDataSet(key);
        DataSetWithKey dswk = new DataSetWithKey(key, dataSet);
        Object existingDataSet = dataSetsAdapter.getDataSet(key);
        // make sure the data set in the initial selection exists in the adapter
        if (existingDataSet == null)
        {
          throw new IllegalArgumentException(
              "No data set found for in data set list for key "+key.toString());
        }
        else if (existingDataSet != dataSet)
        {
          throw new IllegalArgumentException(
              "Initial data set "+existingDataSet.toString()+
              " was not found in data set list");
...
*/
  ...
  private void initialize()
  {
    ...
    overallList.addMouseListener(new MouseAdapter()
    {
      public void mouseClicked(MouseEvent e)
      {
         if (e.getClickCount() >= 2)
         {
            ...
            {
               DefaultUserInteractor.get().notify(DataSetSelector.this,
                   "Incorrect Object Type", "Expected an object of type DataSetWithKey in " +
                   "overallList, but found " + obj.getClass() +"instead.", UserInteractor.ERROR);
            } // else
         }
         ...
         {
           int idx = overallList.getSelectedIndex();
           if (overallList.isSelectedIndex(idx));
           overallList.removeSelectionInterval(idx,idx);
         } */ // if
      } // mouseClicked()
    } // MouseAdapter()
    );
    ...
    OKCancelPanel ocPanel = new OKCancelPanel(
        new ActionListener()
        {
        ...
        public void actionPerformed(ActionEvent ae)
        {
         ...
         for (int i = selObj.length - 1; i >= 0; --i)
         {
            if (selObj[i] instanceof DataSetWithKey)
               ...
               throw new Exception("Selected objects in DataSetSelector list were not instances of DataSetWithKey as " +
...
//  make the dialog come up in the center of the screen
  ...
  /**
   * Add the datasets selected in the "Available Data Sets" window
   * if there are currently no datasets selected.
   */
  private void addSelectedDataSets()
  {
    ...
    {
      e.printStackTrace();
      DefaultUserInteractor.get().notify(this,"Error adding data sets",
                                       e.getMessage(), UserInteractor.ERROR);
      return;
    }
  }
   ...
   * @param setToAdd array of DataSetWithKeys
   * @param alreadySelected Vector of already selected datasets
   */
  private void addDataSetToSelection(DataSetWithKey setToAdd,
     Vector selected) throws Exception
  {
    if ((setToAdd == null) || (selected == null)) return;
    ...
    {
      int result = selectedValStr.compareToIgnoreCase(
          ((DataSetWithKey)selected.elementAt(j)).getName());
      if (result == 0)
      {
        foundPlace = true;
      }
  ...
  private boolean okToAddDataSet(int currentSize)
  throws Exception
  {
    if ((dataSetInfo.getMaxNumber() > 0) &&
        (1 + currentSize > dataSetInfo.getMaxNumber()))
    {
      if (dataSetInfo.getMaxNumber() == 1)
      {
        throw new Exception("Only one data set of this type can be used on this plot");
  ...
  public Vector getSelection()
  {
    if (didOK)
    {
      ...
      //for (int i = selectedDataSetWithKeys.size()-1; i >= 0; i--)
      for (int i = 0; i < selectedDataSetWithKeys.size(); i++)
      {
        DataSetWithKey dswk = (DataSetWithKey)selectedDataSetWithKeys.get(i);
        System.out.println("key "+i+" = "+dswk.key);
        ds.add(dswk.dataSet, dswk.key);
      }
      return ds;*/
  ...
  public static Vector pickDataSets(DataSetsAdapter dataSetsAdapter,
                                      Vector alreadySelected,
                                      DataSetInfo dsInfo)
  {
    DataSetSelector selector = new DataSetSelector(dataSetsAdapter,
        alreadySelected, dsInfo);
