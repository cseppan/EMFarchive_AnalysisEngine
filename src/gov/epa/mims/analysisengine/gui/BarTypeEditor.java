...
/**
 * A Dialog to edit "properties" of a bartype, which is one of the analysis
 * options for a barplot.
 *
 * @author Alison Eyth, Prashant Pai, CEP UNC
 * @version $Id: BarTypeEditor.java,v 1.1 2005/09/19 14:14:04 rhavaldar Exp $
 * @see BarType.java
 * @see BarPlot.java
 * @see AnalysisOptions.java
 */

public class BarTypeEditor
    extends OptionDialog
{
 ...
 /** a check box to indicate whether to sort the labels or not 
  ...
  /** a boolean value panel to check whether the numeric axis should be in log
   * scale
   **/
  ...
  /** a boolean value panel to check whether the bars should be horizontal or
   * vertical **/
  ...
  private void initialize()
  {
    ...
    stackedPanel.setBorder(BorderFactory.createTitledBorder(
      BorderFactory.createBevelBorder(BevelBorder.LOWERED), "Stacking",
      TitledBorder.LEFT, TitledBorder.TOP));
    horizPanel = new BooleanValuePanel("Vertical", "Horizontal", false);
    horizPanel.setBorder(BorderFactory.createTitledBorder(
    ...
    positionPanel.setBorder(BorderFactory.createTitledBorder(
      BorderFactory.createBevelBorder(BevelBorder.LOWERED), "Bar Widths",
      TitledBorder.LEFT, TitledBorder.TOP));
    ...
    BorderFactory.createBevelBorder(BevelBorder.LOWERED), "Bar Colors",
      TitledBorder.LEFT, TitledBorder.TOP));
    ...
    barSpace = new DoubleValuePanel("Space between bars:", false, 0,
...
//System.out.println("enable = "+ enable);
    ...
    TableColumn [] tableColumns =
      {lineStyleColumn,lineWidthColumn,angleColumn,shadeDensityColumn};
  ...
  protected void initGUIFromModel()
  {
    ...
    //RP: A temp solution until Tommy implements a flag to check whether we will have shading or not
     ...
     if(lineStyles != null && lineWidth != null && angle != null &&
     density != null
     && (lineStyles.length == lineWidth.length) &&
      (lineStyles.length == angle.length) &&
    ...
    if (foreColorsPanel.getValue() != null)
    {
       ...
       else
       {
          DefaultUserInteractor.get().notify(this,"Unexpected object type",
            "Expected a Color[] in BarTypeEditor.saveGUIValuesToModel(). " +
