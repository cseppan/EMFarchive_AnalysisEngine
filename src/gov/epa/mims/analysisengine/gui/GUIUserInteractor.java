 ...
 *
 * @author Steve Fine
 * @version $Id: GUIUserInteractor.java,v 1.1 2005/09/19 14:14:04 rhavaldar Exp $
 *
 */
 ...
 *
   ...
   public void notifyOfException(Component parent, String title, Throwable exception, int type)
   {
     ...
     notify(parent,title,
            ((exception instanceof Error || exception instanceof RuntimeException) ?
    ...
    * @exception java.lang.IllegalArgumentException is thrown if 'type' is
    *             invalid
    *
    ******************************************************************************/
   ...
   public static void main(String args[])
   {
      ...
      cs.notify(null,"Two Lines", "This long message should appear on two lines.",
                UserInteractor.WARNING);
   
      cs.notify(null,"Starting Test", "Button numbers start at 0.",
                UserInteractor.NOTE);
   
      String[] s = {"One", "Two", "Three", "Four", "Five" };
      int i = cs.selectOption(null, "Choose option", "Please press one of the buttons below.",
                               s, 0);
      System.err.println("returned a " + i );
   
      String[] s2 = {"Fish", "Fowl", "Pheasant", "Venison", "Poultry", "Crustacean" };
      i = cs.selectOption(null, "Choose option", "Please press one of the buttons below.",
                               s2, 3);
      System.err.println("returned a " + i );
   
      int result = cs.selectOption(null,"Test Choice", "Choose a value and check the " +
                                "printed result.",
                                YES_NO_CANCEL, NO);
      System.out.println(result == YES ? "YES" :
         (result == NO ? "NO" : (result == CANCEL ? "CANCEL" : "no choice")));
   
      result = cs.selectOption(null,"Test Choice Again", "Choose a value and check the " +
                                "printed result.",
                                YES_NO_CANCEL, NO);
      System.out.println(result == YES ? "YES" :
         (result == NO ? "NO" : (result == CANCEL ? "CANCEL" : "no choice")));
