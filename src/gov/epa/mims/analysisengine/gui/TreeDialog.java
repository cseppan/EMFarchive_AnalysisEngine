 ...
 * the user to access multiple pages and plots. Currently, this is a modal
 * dialog that is restricted to trees of the following form:
 *
 * data sets <- page options <- page <- plot options <- plot
 *
 * It will contain buttons:
 * - Select Data, which will bring up the DataSetsAdapter,
 * - Save, which will return a tree containing the info in the dialog
 * - Create Plots, which will take in the information from the dialog and
 *   construct and execute the newly created tree.
 * - Cancel, which will either return the original tree if one was provided or
...
public class TreeDialog extends JDialog
   implements   AnalysisOptionConstantsIfc,  PageConstantsIfc
{
   ...
   static
   {
      ...
      dependentAxisInfo.put(AnalysisEngineConstants.DISCRETE_CATEGORY_PLOT,
         Y_NUMERIC_AXIS);
    ...
    * @param dataSetsAdapter DataSetsAdapter the optional provision to bring
    *   in datasets -- either actual or dummy
   ...
   /*
    * To use as a modal dialog with a specific a plot type and datasetsadapter
    * In this case, no tree is passed in, so we're building a new tree.
    *
    * @param String plotType the type of plot that could be created -
    *    see PlotInfo.getAvailablePlotTypes for valid plot types
    * @param DataSetsAdapter the optional provision to bring in datasets --
    *      either actual or null
    * @param textValues HashMap initial text values to use for plot; keywords
    *      should be standard analysis option keywords; current implementation
    *      supports single page and plot tree only
    * @param rgenerator RGenerator to use to execute trees (could be null)
   ...
   /**
    * constructor with options for supplying a plot type and datasetsadapter
    * In this case, no tree is passed in, so we're building a new tree.
    *
    * @param plotType String the type of plot that could be created -
    *    see PlotInfo.getAvailablePlotTypes for valid plot types
    * @param dataSetsAdapter DataSetsAdapter the optional provision to bring in datasets --
    *      either actual or null
    * @param textValues HashMap initial text values to use for plot; keywords
    *      should be standard analysis option keywords; current implementation
    *      supports single page and plot tree only
   ...
   public void createPlotFromGUI(boolean onScreen)
   {
         ...
         else
         {
            ...
            if (prospectiveFile.exists())
            {
               int retval = JOptionPane.showConfirmDialog(this,
                 "The file "+prospectiveFile+" already exists. Overwrite?",
      ...
      {
         DefaultUserInteractor.get().notifyOfException(this,
               "Error Creating Plot", exc, UserInteractor.ERROR);
    ...
    * @param option String specifying the option to look up
    * @param plot Plot to get text for
   ...
   /*
    * Set the text for the specified analysis options node from the default
    * @param options
    */
...
//         dataSetsAdapter = new DummyDataSetsAdapter();
    ...
    * @param tree Branch that is the root of the tree.
   ...
   /**
    * once the plot button was clicked, a tree will constructed/edited based
    * on the data that was set in the dialog
    */
      ...
      else
      {
         ...
         else
         {
            DefaultUserInteractor.get().notify(this,
               ...
               "Expected a DataSets in TreeDialog.constructTree() but found a " +
               tree.getClass().toString() + " instead.", UserInteractor.ERROR);
         }
      }
   }
   ...
   private Branch constructNewTree()
   {
      ...
      plotOptions = pagePanel.getPlotOptions();
      //System.out.println("Plot Title = "+plotOptions.getOption(gov.epa.mims.analysisengine.tree.AnalysisOptionConstantsIfc.PLOT_TITLE));
   ...
   protected void createPlotOptions()
   {
      if (plotType == null)
      {
         DefaultUserInteractor.get().notify(this,
               "Plot type null",
               "The plotType cannot be null when calling createPlotOptions()!",
    ...
    * @param file File that should e a serialized tree file.
    */
   public void loadTreeFromFile(File file)
   {
      ...
      {
         DefaultUserInteractor.get().notifyOfException(this,
               "Error Reading file", e, UserInteractor.ERROR);
      } // catch
   } // loadTreeFromFile()
   ...
   public void loadTree(byte[] bytes)
   {
     ...
     try
     {
        ObjectInputStream inputStream = new ObjectInputStream(
         ...
         pagePanel.setDataModel(pageOptions, plotOptions, currentPlot,
            dataSetsAdapter);
         setTitle("MIMS Analysis Engine " + plotType);
         pack();
      } // try
      ...
      {
         DefaultUserInteractor.get().notifyOfException(this,
               "Error Reading the array", e, UserInteractor.ERROR);
      } // catch
  }

  /**
   * Try to load the selected file as a serialized verion of the tree.
   * NOTE: We are NOT saving the root DataSets node. We strip the
   *       datasets from the root before serialization.
   * @param file File to open as a serialized tree.
   */
  public void saveTreeToFile(File file)
  {
    ...
    if (tree == null)
    {
       DefaultUserInteractor.get().notify(this,"No Plot Template",
    ...
    try
    {
       ...
       while (writeFile == JOptionPane.NO_OPTION)
       {
          ...
          if (file.exists())
          {
             writeFile = JOptionPane.showConfirmDialog(this,
                "The file " + file.getAbsolutePath() + " already exists. Overwrite?",
                "Overwrite File?", JOptionPane.YES_NO_OPTION,
                JOptionPane.QUESTION_MESSAGE);
          }
          else
    ...
    {
       DefaultUserInteractor.get().notifyOfException(this,
             "Error Writing file", e, UserInteractor.ERROR);
     ...
     */
   public boolean saveGUIValuesToTree(boolean onScreen)
         throws Exception
   {
      boolean shouldDrawPlot = true;
      pagePanel.storeGUIValues(onScreen);
    ...
    /**
     * Show a simple about box with information about the version,
     * authors, etc.
     * Set the version and date in the static variables at the top of the class.
     */
    public void showAboutDialog()
    {
       ...
       DefaultUserInteractor.get().notify(this,
           "About Analysis Engine", sb.toString(), UserInteractor.PLAIN);
    } // showAboutDialog()
   ...
   public void showUserGuide()
   {
      ...
      {
         DefaultUserInteractor.get().notifyOfException(this,
               "Error Loading Help File", ioe, UserInteractor.ERROR);
      }
   }
   ...
   /**
    * Add a new plotInfo object to the global list of plot infos.
    * Each concrete plot class should create a new PlotInfo object and
    * add it to the catalog by calling this method.
    * @param newPlotInfo PlotInfo the new one to add
    ...
    * @param fileChooserType String that is either JFileChooser.OPEN_DIALOG or
    ...
    *
    * @param plotType String the type of plot - should be a constant in this class
    * @return String [] the keywords for the requested type of plot, or null if
    *    the plot type is unknown
    */
   public static PlotInfo getPlotInfoFor(String plotType)
   {
/*
      if (plotType.equalsIgnoreCase(PlotConstantsIfc.BAR_PLOT))
      {
         return (PlotInfo)plotInfoMap.get(AnalysisEngineConstants.BAR_PLOT);
      }
      else if (plotType.equalsIgnoreCase(PlotConstantsIfc.DISCRETE_CATEGORY_PLOT))
      {
         return (PlotInfo)plotInfoMap.get(AnalysisEngineConstants.DISCRETE_CATEGORY_PLOT);
      }
      else if (plotType.equalsIgnoreCase(PlotConstantsIfc.HISTOGRAM_PLOT))
      {
         return (PlotInfo)plotInfoMap.get(AnalysisEngineConstants.HISTOGRAM_PLOT);
      }
      else if (plotType.equalsIgnoreCase(PlotConstantsIfc.RANK_ORDER_PLOT))
      {
         return (PlotInfo)plotInfoMap.get(AnalysisEngineConstants.RANK_ORDER_PLOT);
      }
      else if (plotType.equalsIgnoreCase(PlotConstantsIfc.SCATTER_PLOT))
      {
         return (PlotInfo)plotInfoMap.get(AnalysisEngineConstants.SCATTER_PLOT);
      }
      else if (plotType.equalsIgnoreCase(PlotConstantsIfc.TIME_SERIES_PLOT))
      {
         return (PlotInfo)plotInfoMap.get(AnalysisEngineConstants.TIME_SERIES_PLOT);
      }
      else
      {
*/
...
//      }
    ...
    *
    * @param plot Plot the plot to get info for
   ...
   /**
    * Method to be called from outside of the AnalysisEngine to create a plot
    * without bringing up the GUI. The initial motivation for this is to
    * be able to draw multiple plots and save them without brining up the GUI.
    * @param tree Branch the optional tree which the dialog will work off of
    * @param dataSetsAdapter DataSetsAdapter the optional provision to bring
    *   in datasets -- either actual or dummy
    * @param rgenerator RGenerator to use to execute trees (could be null)
    ...
    *    a new tree if the user pressed Save
    */
   public static Branch createPlotWithoutGUI(Branch tree,
      DataSetsAdapter dataSetsAdapter, RGenerator rgenerator,
      HashMap textValues)
      throws Exception
   {
    ...
    *
    * @param plotType String type of plot to return a plot for
    * @return Plot a new plot, or null if one can't be created
    * @throws Exeption if plot cannot be created
    */
   public static Plot createNewPlot(String plotType)
      throws Exception
   {
...
System.out.println("plotType="+plotType);
      ...
      /* PlotInfo plotInfo = getPlotInfoFor(plotType);
      if (plotInfo != null)
      {
         try
         {
            Plot plot = (Plot)plotInfo.getPlotClass().newInstance();
            return plot;
         }
         catch (Exception exc)
         {
            throw new Exception(" A new plot of type "+plotType+" could not be created");
         }
      }*/
   ...
   public static String getDependentAxis(Component parent,String plotType)
   {
      ...
      if(axisName == null)
      {
         DefaultUserInteractor.get().notify(parent,"Error","This "+ plotType + " is not added to "
         + "the dependendAxisInfo hashMap in the TreeDialog",UserInteractor.ERROR);
      }
      return axisName;
   }
   ...
   public static boolean isLabelRequired(Component parent,String plotType)
   {
      ...
      {
         DefaultUserInteractor.get().notify(parent,"Error","This "+ plotType + " is not added to "
         + "the plotLabelInfo hashMap in the TreeDialog",UserInteractor.ERROR);
      }
      return req.booleanValue();
   }

   /**
    * Given a tree, traverse it and return an object that cn be queried about how
    * many pages and plots th tre contains.
    *
    * @param tree Branch that is the root of the tree.
    * @return PageAndPlotSummary that contains information about how many
    * page and plots the tree contains.
    */
   ...
   /**
    * A method that accepts a File that is an Analysis Engine serialized tree and
    * returns the tree if reading was successful.
    * @param file File that should be read in.
    * @returns Branch that is the root of a tree fora plot.
    * @throws IOException if the file reading was unsuccessful.
    */
    public static Branch readTreeFile(File file)
   ...
   public static void main(String[] args)
   {
      ...
      textValues.put(
         gov.epa.mims.analysisengine.tree.AnalysisOptionConstantsIfc.PLOT_TITLE,
         "A specified title");
      textValues.put(
         gov.epa.mims.analysisengine.tree.AnalysisOptionConstantsIfc.PLOT_SUBTITLE,
         "A specified subtitle");
      textValues.put(
         gov.epa.mims.analysisengine.tree.AnalysisOptionConstantsIfc.PLOT_FOOTER,
         "A footer");
      textValues.put(
         gov.epa.mims.analysisengine.tree.AnalysisOptionConstantsIfc.CATEGORY_AXIS,
         "Category Axis");
      textValues.put(
         gov.epa.mims.analysisengine.tree.AnalysisOptionConstantsIfc.NUMERIC_AXIS,
         "Numeric Axis");
      textValues.put(
         gov.epa.mims.analysisengine.tree.AnalysisOptionConstantsIfc.X_NUMERIC_AXIS,
         "X Numeric Axis");
      textValues.put(
         gov.epa.mims.analysisengine.tree.AnalysisOptionConstantsIfc.Y_NUMERIC_AXIS,
         "Y Numeric Axis");
      textValues.put(
         gov.epa.mims.analysisengine.tree.AnalysisOptionConstantsIfc.X_TIME_AXIS,
         "X Time Axis");

      textValues.put(
         gov.epa.mims.analysisengine.tree.AnalysisOptionConstantsIfc.PAGE_TYPE,
         "tmp.png");
      ...
      else  // (showGUI == false)
      {
         ...
         try
         {
            branch = TreeDialog.createPlotWithoutGUI(tree, null,
                  null, textValues);
         }
         catch (Exception e)
         {
