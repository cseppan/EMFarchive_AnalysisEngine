...
/**
 * A Panel for setting up a plot. This will display the plot type and
 * contains a table for editing page options.
 *
 * @author Alison Eyth
 * @version $Id: PlotPanel.java,v 1.1 2005/09/19 14:14:04 rhavaldar Exp $
 *
 **/
   ...
   extends JPanel
{
   ...
   /**The border around the options panel. We need to update the plot type
    * it the plot type changes. (ie if a new serialized tree comes in) */
   protected TitledBorder optionsBorder = null;


   /** True if we need to reset the dataSetsPanel when a new tree is read
    * in froma file. This will occur if the user starts with a BarPlot and
    * then reads in a ScatterPlot tree. */
    ...
    *
    * @param plotType String type of plot to create
    * @param dataSetsAdapter DataSetsAdapter to select data from
    * @exception Exception if the plot cannot be created
    */
   public PlotPanel(String plotType, DataSetsAdapter dataSetsAdapter )
   {
      ...
      {
        throw new IllegalArgumentException(
          "No PlotInfo available for plot type "+plotType);
      }
   ...
   private void initialize()
   {
      ...
      optionsBorder = BorderFactory.createTitledBorder(
            BorderFactory.createLoweredBevelBorder(), plotType + " Options",
      ...
      dataSetsPanel.setBorder(BorderFactory.createTitledBorder(
            BorderFactory.createLoweredBevelBorder(),
            "Data sets", TitledBorder.LEFT, TitledBorder.TOP));
      dataSetsTable = new DataSetsTable(dataSetsAdapter, plotInfo);
    ...
    *
    * @param newPlotOptions AnalysisOptions to use as the new data model.
    * @param newPlot Plot to use as the new data model.
    * @param newDataSetsAdapter DataSetsAdapter to use as the new data model.
    */
   protected void setDataModel(AnalysisOptions newPlotOptions,
                               Plot newPlot, DataSetsAdapter newDataSetsAdapter)
   {
