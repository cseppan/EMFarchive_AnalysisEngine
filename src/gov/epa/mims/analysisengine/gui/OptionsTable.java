...
/**
 * A table to display, edit and save options, containing name value pairings.
 * There will be columns for the name, value, and information about the value.
 *
 * The value editor would vary depending on the type of value defined in the
 * corresponding optionInfo. This table can be used for page options as well
 * as plot options.  The copy, paste and clear actions are implemented in
 * this table, but the buttons are located on the OptionsPanel toolbar.
 *
 * @author Alison Eyth
 * @version $Id: OptionsTable.java,v 1.1 2005/09/19 14:14:04 rhavaldar Exp $
 *
  ...
  extends JTable
{
   ...
   /** The type of plot that this table is representing. Must be one of the plot
    * type constants from AnalysisEngineConstants. */
   private String plotTypeName = null;


    /**
    * Constructor for options table.
    * @param options AnalysisOptions to edit
    * @param allKeywords String [] the list of all possible keywords
    * @param defaultValues Object [] default values for all options
    * @param plotType String that is one of the plot constants in AnalysisEngineConstants.
    */
   public OptionsTable(AnalysisOptions options, String [] allKeywords,
      AnalysisOption [] defaultValues, String plotType)
   {
      ...
      for (int i = 0; i < allKeywords.length; i++)
      {
         try
         {
            ...
            else
            {
               ...
               if (editorClass == null)
               {
                 ...
                 System.err.println("Could not find value editor type for "+
                                    allKeywords[i]);
               }
               ...
               {  /* this is the main branch that should be taken */
                  editor = new OptionTableCellEditor();
                  ((OptionTableCellEditor)editor).initFromOptionInfo(oi,
                     allValues[i], plotTypeName);
               }
            }
            editors.add(editor);
         }
         catch (Exception exc)
         {
            ...
            throw new IllegalArgumentException(
               "Could not find editor for option "+ allKeywords[i]);
         }
      }
   }
   ...
   protected void clearValues()
   {
      ...
      for (int i = 0; i < rows.length; i++)
      {
         ...
         try
         {
            ((OptionTableCellEditor)editors.get(thisRow)).resetOption(
               (OptionInfo)optionInfos.get(thisRow));
         }
         catch (Exception exc)
         {
            exc.printStackTrace();
   ...
   protected void updateOption(int row, int col, OptionInfo optionInfo,
      Object objectToEdit)
   {
    ...
    */
   private class OptionsTableModel
   extends AbstractTableModel
   {
      ...
      public int getRowCount() { return allKeywords.length; }
