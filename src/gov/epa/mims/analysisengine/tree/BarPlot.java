 ...
 * </pre>
 *
 * <p><A HREF="doc-files/ExampleBarPlot01.html"><B>ExampleBarPlot01.html</B></A>
 * <p><A HREF="doc-files/ExampleBarPlot02.html"><B>ExampleBarPlot02.html</B></A>
 * <p><A HREF="doc-files/ExampleBarPlot03.html"><B>ExampleBarPlot03.html</B></A>
 *
 *
              ...
              AnalysisOptionConstantsIfc
{
    ...
    * {@link BarPlot#setDataSetKeys(Object[])}
    */
   ...
   {
      /*
       String name = "Bar Plot Options";
       Class valueType = BarType.class;
       String toolTipText = null;
       Class valueEditorType = BarTypeEditor.class;
       OptionInfo optionInfo = new OptionInfo(name, valueType, toolTipText,
                                              valueEditorType);
       OptionInfo.addKeywordAndInfo(BAR_TYPE,
                                    optionInfo);
       */
   }
   ...
   **/
    ...
    *
    * @param i index of key to retrieve
    *
   ...
   /**
    * create the data set keys from an array of dataSets
    * then call setDataSetKeys
...
System.out.println("the datasets size ="+dataSets.size());
      ...
      {
         throw new Exception(
           "Only one set of data sets should be provided to the Bar Plot, but "+
           dataSets.size()+" were provided");
      }
/*System.out.println("In createDataSetKeys() class ="+ dataSets.get(0).getClass());
      Vector tmpDataSet = (Vector)dataSets.get(0);
System.out.println("the tmpDataSet class = "+tmpDataSet.get(0).getClass());
      Vector data = ((DataSets)tmpDataSet.get(0)).getDataSets(null, null);
      for(int i=0; i< data.size(); i++)
      {

         LabeledDataSetIfc dateSetIfc = (LabeledDataSetIfc)data.get(i);
         dateSetIfc.open();
         int length = dateSetIfc.getNumElements();
         if(length > MAX_ELEMENTS)
         {
            throw new Exception("There were "+length+
              " data points in one of the data sets passed to Bar Plot, but only "+
               MAX_ELEMENTS + "can be plotted.");
         }
      }
 */
   ...
   private static void initPlotInfo()
   {
      ...
      dataSetInfos[0] = new DataSetInfo("Bar Data Series", DataSetIfc.class, 1,
                                        -1);
      plotInfo = new PlotInfo(AnalysisEngineConstants.BAR_PLOT, BarPlot.class,
